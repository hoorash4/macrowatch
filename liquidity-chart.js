  ...
  function render(card, state) {
    ...
    host.innerHTML = ['pressure', 'capacity'].map(metric => {
      ...
      rows.forEach(r => {
        ...
        if (previous !== null && time - previous > maxGap * DAY) points.push({x:NaN,y:NaN});
      ...
      return `<div class="mt-4"><div class="flex flex-wrap justify-between gap-2 text-sm"><h3 style="color:${colors[metric]}">${names[metric]}</h3><span class="text-slate-500">${labelDate} · ${Number(last.score).toFixed(1)} / 100 · ${frequency}${stale?' · 갱신 지연':''}</span></div><svg data-metric="${metric}" viewBox="0 0 ${width} ${height}" style="width:100%;min-height:150px" role="img" aria-label="${names[metric]} ${frequency} 추이"><title>${names[metric]}: ${labelDate}, ${Number(last.score).toFixed(1)}점</title>${grids}${ticks}<path d="${utils.monotonePath(points)}" fill="none" stroke="${colors[metric]}" stroke-width="2"/><line data-cursor x1="0" x2="0" y1="${top}" y2="${height-bottom}" stroke="#64748b" visibility="hidden"/></svg><p data-detail="${metric}" class="text-xs text-slate-500" style="min-height:1.3em">${rows.some(r=>r.is_warmup)?'초기 표본이 적은 구간 포함 · 날짜를 가리키면 표본 수 표시':'날짜를 가리키면 점수와 표본 수 표시'}</p></div>`;
    }).join('');
    host.querySelectorAll('svg[data-metric]').forEach(svg => {
      ...
      svg.addEventListener('pointermove', event => {
        ...
        host.querySelector(`[data-detail="${svg.dataset.metric}"]`).textContent = `${nearest.frequency==='M'?nearest.observation_date.slice(0,7):nearest.observation_date} · ${Number(nearest.score).toFixed(1)}점 · 표본 ${nearest.sample_count}개${nearest.is_warmup?' · 초기 표본 부족':''}`;
      });
  ...
  async function load({supabaseClient}) {
    ...
    await Promise.all([...document.querySelectorAll('[data-liquidity-country]')].map(async card => {
      ...
      const {data,error} = await utils.loadAllRows((from,to)=>supabaseClient.from('liquidity_indices')
        .select('observation_date,metric,score,frequency,sample_count,is_warmup').eq('country',country)
