...
OUT = Path("tmp_pivot_charts_rerun")
...
def render(code: str, rows, freq: str, start: date, end: date):
    ...
    final_rdp = augment_spike_entry_points(base, geometry)
    final = simplify_pivot_lines(final_rdp, geometry)
    ...
    ax.set_title(f"{code} | {freq} | full ±24M buffer | spike-corrected final RDP")
    ...
    print(code,
          "range=", start, end,
          "rows=", len(rows),
          "base_rdp=", len(base.display_markers),
          "final_rdp=", len(final_rdp.display_markers),
          "spikes=", len(final_rdp.spike_peaks),
