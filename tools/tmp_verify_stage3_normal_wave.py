...
OUT=Path("tmp_stage3_normal_wave_four")
...
def run(code,rows,freq,start,end):
    ...
    # Strict one-way stage boundaries.
    assert {pkey(p) for p in s3.markers} <= {pkey(p) for p in s2.display_markers}
    assert {pkey(p) for p in s4.markers} <= {pkey(p) for p in s3.markers}
    assert {pkey(p) for p in s5.markers} <= {pkey(p) for p in s4.markers}

    # Final renderer receives RAW + final stage only. No envelope/candidates/RDP debug data.
    ...
    ax.set_title(f"{code} | raw + Stage5 final only")
    ...
    print(code,"S3_POINTS",[(str(p.day),p.value,p.pivot_type) for p in s3.markers])
    print(code,"FINAL",[(str(p.day),p.value,p.pivot_type) for p in s5.markers])
