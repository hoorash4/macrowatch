...
OUT = Path("tmp_adjacent_final_four")
...
def render(code, rows, freq, start, end):
    ...
    assert final_keys <= stage1_keys, f"{code}: resurrected non-stage1 point"
    for seg in final.segments:
        assert pkey(seg.start) in final_keys
        assert pkey(seg.end) in final_keys

    if code == "US10Y_REAL":
        circled = (date(2022, 9, 30), 1.68, "high")
        assert circled in final_keys, "US10Y_REAL 2022-09-30 high was incorrectly deleted"
