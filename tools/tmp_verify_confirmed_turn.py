...
OUT = Path("tmp_confirmed_turn_four")
...
def render(code, rows, freq, start, end):
    ...
    for seg in final.segments:
        assert pkey(seg.start) in final_keys and pkey(seg.end) in final_keys

    if code == "US10Y_REAL":
        assert (date(2018,11,27), 1.15, "high") in final_keys
        assert (date(2022,9,30), 1.68, "high") in final_keys
