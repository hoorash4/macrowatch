...
DISPLAY_START = date(2019, 10, 11)
DISPLAY_END = date(2023, 6, 8)
WIDTH = 1200.0
HEIGHT = 600.0
OUT = Path("tmp_pivot_charts")
...
def y_geometry(rows):
    ...
    visible = [p.value for p in pts if DISPLAY_START <= p.day <= DISPLAY_END]
    if not visible:
        visible = [p.value for p in pts]
    lo, hi = min(visible), max(visible)
    span = hi - lo
    pad = span * 0.08 if span else max(abs(hi) * 0.08, 1.0)
    return ChartGeometry(DISPLAY_START, DISPLAY_END, lo - pad, hi + pad, WIDTH, HEIGHT)


def draw(series_code: str, rows, frequency: str):
    raw = normalize_rows(rows)
    base = calculate_base_pivots(rows, frequency)
    geometry = y_geometry(rows)
    ...
    ax.set_title(f"{series_code} | {frequency} | simplified pivot line")
    ...
    plt.close(fig)
    print(series_code, "rows=", len(rows), "raw_last=", raw[-1].day, "segments=", len(simplified.segments), "markers=", len(simplified.markers))


def main():
    db = SupabaseRest()
    for code in ("US10Y_REAL", "NFCI", "US_COMMERCIAL_CH11"):
        rows, frequency, _, _ = load_case_series(
            db,
            case_code=CASE_CODE,
            index_code=INDEX_CODE,
            series_code=code,
        )
        draw(code, rows, frequency)

    rows, frequency, _, _ = derive_us10y2y(db)
    draw("US10Y2Y", rows, frequency)
