...
OUT = Path("tmp_stage3_rebuild_four")
OUT.mkdir(exist_ok=True)

def key(p):
    return (p.day, round(p.value, 6), p.pivot_type)
...
def run(code,rows,freq,start,end):
    ...
    s2keys={key(p) for p in s2.display_markers}
    assert {key(p) for p in s3.markers} <= s2keys
    assert {key(p) for p in s4.markers} <= {key(p) for p in s3.markers}
    assert {key(p) for p in s5.markers} <= {key(p) for p in s4.markers}

    if code=="NFCI":
        s3keys={key(p) for p in s3.markers}
        assert (date(2018,3,30),-0.497,"high") in s3keys, "first downtrend high must survive Stage3"
    ...
    ax.set_title(f"{code} | rebuilt Stage3")
