...
def run(code,rows,freq,start,end):
    ...
    if code=="US10Y2Y":
        ...
        assert (date(2000,10,13),-0.11,"high") in s4keys
        ...
        assert (date(2008,9,8),1.36,"low") not in s4keys
    ...
    if code=="US10Y_REAL":
        ...
        print("US10Y_REAL_S4_SEG",[(str(x.start.day),x.start.value,x.start.pivot_type,str(x.end.day),x.end.value,x.end.pivot_type,x.kind) for x in s4.segments if x.end.day.year>=2021])
    ...
    ax.set_title(f"{code} | Stage4 10-degree fix")
