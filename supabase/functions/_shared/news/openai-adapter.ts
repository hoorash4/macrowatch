...
## [결정적 뉴스 사건 키]
결정적 뉴스(extreme_signal=decisive)에는 같은 실제 사건을 여러 기사에서 하나로 묶기 위한 extreme_event_key를 반드시 작성한다. extreme_event_key는 기사의 영향이나 해석이 아니라 "무슨 실제 사건이 발생했는가"를 식별하는 중복 집계용 키다.

규칙:
1. 기사 표현, 제목, 강조점, 해설, 시장 영향 경로가 달라도 핵심 주체와 핵심 행위·결정·발생 사실, 사건의 시점 또는 상태가 실질적으로 같다면 동일한 실제 사건으로 본다.
2. 동일한 실제 사건을 다루는 기사들은 그 기사마다 강조하는 결과나 파급 경로가 달라도 반드시 동일한 extreme_event_key를 사용한다.
3. extreme_event_key에는 사건 자체만 표현한다. 유가 급등, 자금시장 경색, 주택시장 경색, 시장 불안, 위험회피, 인플레이션 압력, 금리 인상 우려처럼 사건의 결과·영향·시장 반응·해석은 넣지 않는다. 이런 내용은 extreme_keywords에 기사별로 반영할 수 있다.
4. 입력의 existing_decisive_event_keys 중 같은 실제 사건을 나타내는 키가 있으면 현재 기사 표현이 달라도 반드시 그 기존 키를 정확히 재사용한다. 같은 사건의 후속 보도, 재인용, 분석, 영향 보도, 상태 지속 보도라는 이유로 새 키를 만들지 않는다.
5. 반대로 같은 주제·원인·업종·정책 방향·시장 영향만 공유할 뿐 핵심 주체, 핵심 행위·결정·발생 사실, 사건의 시점 또는 상태가 실질적으로 다르면 별개의 사건으로 보고 서로 다른 extreme_event_key를 사용한다.
6. 실제로 별개의 사건이 새로 발생한 경우에만 새 extreme_event_key를 만든다.
7. 동일한 extreme_event_key를 사용하더라도 extreme_keywords는 각 기사가 강조하는 시장 영향 경로에 따라 서로 달라도 된다.
