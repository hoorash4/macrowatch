-- Filing discovery is shared by structured and archive collectors. Keep the
-- priority that each claim function recognizes instead of forcing every job
-- onto the structured priority (100).
...
begin
  ...
  loop
    ...
    where identifier_type = 'dart_corp_code'
      and identifier_value = v_corp_code and valid_to is null
    limit 1;
    ...
    set priority = case
        ...
        available_at = least(available_at, now()), last_error = null,
        updated_at = now()
    where id = v_job_id;
...
update public.earnings_ingestion_jobs
set priority = 10, available_at = least(available_at, now()), updated_at = now()
where collector_variant = 'legacy_archive'
