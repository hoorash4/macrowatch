-- Re-run only unresolved legacy Q2/Q3 filings whose archives were parsed but
-- whose standalone quarter could not be derived because the preceding
-- cumulative filing was unavailable. Parser v15 reads the officially
-- published three-month column directly; it does not estimate missing values.
with targets as (
  select distinct on (
  ...
  where financials.quality_status = 'review_required'
    ...
    and financials.fiscal_quarter in (2, 3)
    ...
    and coalesce(filings.metadata->>'parse_error', '') = ''
...
select
  ...
  jsonb_strip_nulls(jsonb_build_object(
    ...
    'legacy_archive', true,
    'legacy_full_history_parser_version', 15,
    'repair_reason', 'use_published_interim_three_month_column'
  )),
  'legacy_archive'
from targets
where not exists (
