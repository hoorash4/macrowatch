...
begin
  ...
  -- Only this administrator-approved deletion may pass the manual-row guard.
...
$function$;
-- Remove only existing deletion markers; leave every active administrator pivot intact.
do $cleanup$
begin
  perform set_config('request.jwt.claim.role','service_role',true);
  perform set_config('macrowatch.explicit_manual_pivot_delete','true',true);
  delete from public.historical_indicator_manual_pivots where is_deleted=true;
  perform set_config('macrowatch.explicit_manual_pivot_delete','false',true);
end;
$cleanup$;
commit;
