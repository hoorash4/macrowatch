-- One-time full-history repair after validating parser v8 against the
-- published SK Innovation consolidated and separate income statements.
-- Detailed statement tables take precedence over rounded summary tables and
-- ``순이익(손실)`` is recognized as the official net-income line.
...
select
  ...
  jsonb_strip_nulls(jsonb_build_object(
    ...
    'legacy_archive', true,
    'legacy_full_history_parser_version', 8,
    'repair_reason', 'prefer_detailed_statement_and_parse_net_income_loss'
  )),
  'legacy_archive'
