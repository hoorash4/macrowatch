...
-- A moved manual pivot is identified only by its current date.
do $check$
begin
  if exists (
    select 1 from public.historical_indicator_manual_pivots m
    join public.historical_indicator_manual_pivots n
      on n.case_code=m.case_code and n.series_code=m.series_code
      and n.source_date=m.pivot_date and n.source_date<>m.source_date
    where m.source_date<>m.pivot_date
  ) then raise exception 'Manual pivot destination source date already exists'; end if;
end;
$check$;

update public.historical_indicator_manual_pivots
  set source_date=pivot_date
  where source_date<>pivot_date;
