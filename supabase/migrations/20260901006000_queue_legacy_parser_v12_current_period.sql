-- Re-run the legacy window after parser v12 stopped treating every four-value
-- interim row as a standalone/YTD pair. Some older tables instead contain
-- current YTD, prior-year YTD and two annual comparison columns.
...
select 'open_dart', 'financial_period', latest.company_id, latest.fiscal_year,
       ...
       jsonb_strip_nulls(jsonb_build_object(
         ...
         'legacy_archive', true,
         'legacy_full_history_parser_version', 12,
         'repair_reason', 'select_current_ytd_from_multi_period_statement'
       )),
       'legacy_archive'
