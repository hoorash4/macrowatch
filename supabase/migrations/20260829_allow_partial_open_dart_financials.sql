-- Preserve valid OpenDART core metrics even when a company has no single
-- manufacturing-style revenue account.  This is common for banks, insurers,
-- and securities firms; it must not suppress their operating or net income.
...
-- Re-read only historical review items whose canonical row was discarded.
-- Once a partial or complete row exists, rerunning this idempotent migration
-- does not enqueue the period again.
insert into public.earnings_ingestion_jobs
  (source, job_kind, company_id, business_year, report_code, reason, priority, metadata)
select 'open_dart', 'financial_period', repair.company_id, repair.fiscal_year,
       repair.source_report_code, 'repair', 80,
       jsonb_build_object(
         'receipt_no', repair.source_filing_id,
         'filed_on', repair.filing_date,
         'report_name', repair.metadata->>'report_name',
         'repair_partial_core_metrics', true
       )
from (
  select distinct on (filings.company_id, filings.fiscal_year, filings.fiscal_quarter)
    filings.company_id, filings.fiscal_year, filings.fiscal_quarter,
    filings.source_report_code, filings.source_filing_id, filings.filing_date, filings.metadata
  from public.earnings_filings filings
  join public.earnings_companies companies on companies.id = filings.company_id
  where filings.source = 'open_dart' and companies.country = 'KR'
    and filings.fiscal_year >= extract(year from current_date)::integer - 6
    and jsonb_array_length(coalesce(filings.metadata->'missing_metrics', '[]'::jsonb)) > 0
  order by filings.company_id, filings.fiscal_year, filings.fiscal_quarter,
           filings.filing_date desc, filings.created_at desc
) repair
where repair.source_report_code in ('11013', '11012', '11014', '11011')
  and not exists (
    select 1 from public.earnings_quarterly_financials financials
    where financials.company_id = repair.company_id
      and financials.fiscal_year = repair.fiscal_year
      and financials.fiscal_quarter = repair.fiscal_quarter
  )
  and not exists (
    select 1 from public.earnings_ingestion_jobs jobs
    where jobs.source = 'open_dart' and jobs.job_kind = 'financial_period'
      and jobs.company_id = repair.company_id
      and jobs.business_year = repair.fiscal_year
      and jobs.report_code = repair.source_report_code
