...
begin
  ...
  if p_is_deleted then
    ...
    if p_pivot_date is null or p_pivot_value is null
      ...
      or (p_key_reference is not null and p_key_reference not in (
        'START', 'PEAK', 'TROUGH', 'AUTO',
        'START_REF', 'PEAK_REF', 'TROUGH_REF',
        'VERIFIED', 'START_VERIFIED', 'PEAK_VERIFIED', 'TROUGH_VERIFIED',
        'UNCLEAR', 'UNCLEAR_VERIFIED'
      )) then
    ...
    if p_key_reference in ('START', 'PEAK', 'TROUGH') then
      if exists (
        ...
        where m.case_code = p_case_code and m.series_code = p_series_code
          and m.source_date <> p_source_date
          and m.key_references->>p_index_code in (p_key_reference, p_key_reference || '_VERIFIED')
      ) then
    ...
    elsif p_key_reference in ('START_REF', 'PEAK_REF', 'TROUGH_REF', 'VERIFIED', 'START_VERIFIED', 'PEAK_VERIFIED', 'TROUGH_VERIFIED', 'UNCLEAR', 'UNCLEAR_VERIFIED') then
      if p_key_reference in ('START_VERIFIED', 'PEAK_VERIFIED', 'TROUGH_VERIFIED') then
        if exists (
          select 1 from public.historical_indicator_manual_pivots m
          where m.case_code = p_case_code and m.series_code = p_series_code
            and m.source_date <> p_source_date
            and m.key_references->>p_index_code in (replace(p_key_reference, '_VERIFIED', ''), p_key_reference)
        ) then
          raise exception 'Key pivot already exists for this indicator and reference';
        end if;
      end if;
    ...
    elsif p_key_reference = 'AUTO' then
      -- AUTO leaves p_index_code stripped from next_keys so automatic calculation can score it
      null;
    end if;

    v_is_verified := coalesce(
      p_is_verified,
      case
        when p_key_reference = 'VERIFIED' or p_key_reference like '%_VERIFIED' then true
        when p_key_reference is not null and p_key_reference not in ('AUTO') then false
        else null
      end
    );
    ...
    if v_is_verified = false then
      declare
        ...
        cleaned jsonb := '{}'::jsonb;
      begin
        for k, v in select key, value #>> '{}' from jsonb_each(next_keys) loop
          if v = 'VERIFIED' then
            if k <> p_index_code then
              cleaned = cleaned || jsonb_build_object(k, next_keys->k);
            end if;
          elsif v like '%_VERIFIED' then
            cleaned = cleaned || jsonb_build_object(k, replace(v, '_VERIFIED', ''));
          else
            cleaned = cleaned || jsonb_build_object(k, next_keys->k);
          end if;
        end loop;
    ...
    elsif v_is_verified = true then
      declare
        k text;
        v text;
        upgraded jsonb := '{}'::jsonb;
      begin
        for k, v in select key, value #>> '{}' from jsonb_each(next_keys) loop
          if v in ('START', 'PEAK', 'TROUGH', 'UNCLEAR') then
            upgraded = upgraded || jsonb_build_object(k, v || '_VERIFIED');
          else
            upgraded = upgraded || jsonb_build_object(k, next_keys->k);
          end if;
        end loop;
        next_keys := upgraded;
      end;
...
-- Fix the erroneously suppressed 'false' on 1994-12-23 for NFCI_RISK on NASDAQ_COMPOSITE
update public.historical_indicator_manual_pivots
set key_references = key_references - 'NASDAQ_COMPOSITE'
where case_code = 'asian_financial_crisis' and series_code = 'NFCI_RISK' and source_date = '1994-12-23'
  and key_references->>'NASDAQ_COMPOSITE' = 'false';
