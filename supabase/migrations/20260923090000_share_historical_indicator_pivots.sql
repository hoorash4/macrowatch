-- One administrator decision per case, indicator and original date. The origin index
-- remains on the existing row; only the explicit key designation is index-specific.
begin;

alter table public.historical_indicator_manual_pivots
  add column if not exists key_references jsonb not null default '{}'::jsonb;

update public.historical_indicator_manual_pivots
set key_references=jsonb_build_object(index_code,key_reference)
where key_reference is not null and key_references='{}'::jsonb;

alter table public.historical_indicator_manual_pivots
  add constraint historical_manual_key_references_object
  check (jsonb_typeof(key_references)='object');

create unique index historical_manual_source_shared_unique
  on public.historical_indicator_manual_pivots(case_code,series_code,source_date);
create unique index historical_manual_date_shared_unique
  on public.historical_indicator_manual_pivots(case_code,series_code,pivot_date)
  where pivot_date is not null;

-- The legacy column is retained for historical provenance, but key uniqueness now
-- belongs to the selected market index inside key_references.
drop index if exists public.historical_manual_pivot_key_unique;
create unique index historical_manual_start_sp500_unique
  on public.historical_indicator_manual_pivots(case_code,series_code,(key_references->>'SP500'))
  where not is_deleted and key_references ? 'SP500';
create unique index historical_manual_start_nasdaq_unique
  on public.historical_indicator_manual_pivots(case_code,series_code,(key_references->>'NASDAQ_COMPOSITE'))
  where not is_deleted and key_references ? 'NASDAQ_COMPOSITE';
create unique index historical_manual_start_kospi_unique
  on public.historical_indicator_manual_pivots(case_code,series_code,(key_references->>'KOSPI'))
  where not is_deleted and key_references ? 'KOSPI';
...
begin
  if exists (
    ...
    where m.case_code=new.case_code and m.series_code=new.series_code
...
begin
  ...
  -- Serialize all manual decisions for a case/indicator with automatic replacement.
  ...
  if p_pivot_date is null or p_pivot_value is null
    ...
    or p_relationship not in ('positive','inverse')
  ...
  if p_key_reference is not null then
    if exists (
      select 1 from public.historical_indicator_manual_pivots m
      where m.case_code=p_case_code and m.series_code=p_series_code
  ...
  values (p_case_code,origin_index,p_series_code,p_source_date,p_pivot_date,p_pivot_value,
    p_relationship,trim(p_reason),nullif(trim(p_comment),''),next_keys,false,
    automatic_snapshot,p_user_id,p_user_id)
  on conflict (case_code,index_code,series_code,source_date) do update set
