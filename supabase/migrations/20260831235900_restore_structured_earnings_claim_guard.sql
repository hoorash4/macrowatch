...
-- Undo attempts consumed by the overwritten claim function during this repair.
update public.earnings_ingestion_jobs
set status = 'retry', attempts = 0, claimed_at = null, available_at = now(),
    last_error = null, updated_at = now()
where source = 'open_dart' and job_kind = 'financial_period'
  and collector_variant = 'legacy_archive'
  and last_error = 'OpenDART financial data requires a business year from 2015 onward.';
