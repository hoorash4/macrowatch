-- OpenDART's structured endpoints return no rows for much of the 2015
-- universe. Rebuild that YoY baseline with the corrected official filing-
-- archive parser. The deployment restores the 2016 structured boundary and
-- parallel 2002-2015 legacy claim function before exposing this repair RPC.

create or replace function public.enqueue_earnings_legacy_2015_parser_v2_repair()
...
begin
  ...
  set collector_variant = 'legacy_archive', reason = 'repair', priority = 10,
      ...
      metadata = jobs.metadata || jsonb_build_object(
        'legacy_2015_parser_version', 2,
        'legacy_archive', true,
        'repair_reason', 'correct_shifted_interim_cumulative_columns'
      ), updated_at = now()
  from targets
  ...
  where not exists (
    ...
    where jobs.source = 'open_dart' and jobs.job_kind = 'financial_period'
      ...
      and (
        ...
        or jobs.metadata @> '{"legacy_2015_parser_version": 2}'::jsonb
      )
  );
...
revoke all on function public.enqueue_earnings_legacy_2015_parser_v2_repair()
  from public, anon, authenticated;
grant execute on function public.enqueue_earnings_legacy_2015_parser_v2_repair()
  to service_role;

comment on function public.enqueue_earnings_legacy_2015_parser_v2_repair() is
  'Idempotently rebuilds the 2015 YoY baseline after correcting legacy interim cumulative-column selection.';
