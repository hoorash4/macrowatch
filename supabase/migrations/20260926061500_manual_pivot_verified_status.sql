...
begin
  ...
  if p_is_deleted then
    ...
    if p_pivot_date is null or p_pivot_value is null
      ...
      or (p_key_reference is not null and p_key_reference not in ('START', 'PEAK', 'TROUGH', 'AUTO', 'START_REF', 'PEAK_REF', 'TROUGH_REF', 'VERIFIED', 'START_VERIFIED', 'PEAK_VERIFIED', 'TROUGH_VERIFIED')) then
    ...
    elsif p_key_reference in ('START_REF', 'PEAK_REF', 'TROUGH_REF', 'VERIFIED', 'START_VERIFIED', 'PEAK_VERIFIED', 'TROUGH_VERIFIED') then
      next_keys = next_keys || jsonb_build_object(p_index_code, p_key_reference);
