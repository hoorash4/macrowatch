-- Extend the existing public read-only earnings chart endpoints to the two
-- United States index universes. Operational tables remain private.
create or replace function public.earnings_v2_public_latest_company_options()
returns table (
  company_id text,
  company_name text,
  market_id text,
...
as $$
  with latest_periods as (
    ...
    where market_id in ('kr_largecap', 'kr_kosdaq', 'us_sp100', 'us_nasdaq100')
    group by market_id
  )
  select
    u.company_id,
  ...
  order by
    case u.market_id
      when 'kr_largecap' then 1
      when 'kr_kosdaq' then 2
      when 'us_sp100' then 3
      when 'us_nasdaq100' then 4
    end,
    u.market_cap_rank;
$$;

create or replace function public.earnings_v2_public_company_series(p_company_id text)
returns table (
  fiscal_year integer,
