-- Filing discovery legitimately updates metadata and priority. Keep the
-- transport/parser route in an immutable first-class column instead.
alter table public.earnings_ingestion_jobs
  add column if not exists collector_variant text not null default 'structured'
  check (collector_variant in ('structured', 'legacy_archive'));
...
where source = 'open_dart' and business_year between 2002 and 2015
  and (
    ...
    or (
      ...
      and coalesce(metadata->>'receipt_no', '') <> ''
    )
  );

create index if not exists earnings_ingestion_jobs_variant_claim_idx
  on public.earnings_ingestion_jobs
    (collector_variant, status, company_id, business_year, id)
