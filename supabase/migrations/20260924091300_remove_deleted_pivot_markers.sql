...
-- Active administrator pivots have complete payloads; deletion no longer stores a row.
alter table public.historical_indicator_manual_pivots
  drop constraint historical_manual_pivot_payload;
alter table public.historical_indicator_manual_pivots
  add constraint historical_manual_pivot_payload
  check (pivot_date is not null and pivot_value is not null);

drop index public.historical_manual_start_sp500_unique;
drop index public.historical_manual_start_nasdaq_unique;
drop index public.historical_manual_start_kospi_unique;
create unique index historical_manual_start_sp500_unique
  ...
  where (key_references->>'SP500') in ('START','PEAK','TROUGH');
create unique index historical_manual_start_nasdaq_unique
  ...
  where (key_references->>'NASDAQ_COMPOSITE') in ('START','PEAK','TROUGH');
create unique index historical_manual_start_kospi_unique
  ...
  where (key_references->>'KOSPI') in ('START','PEAK','TROUGH');
...
declare
  automatic_snapshot jsonb := '[]'::jsonb;
  prior record;
  next_keys jsonb;
  origin_index text;
  target_date date;
...
begin
  ...
  -- The selected date alone is removed. Moving a manual date cannot delete
  -- a different automatic pivot that happens to occupy the destination date.
  ...
  update public.historical_indicator_ai_scores s
    ...
    where s.case_code=p_case_code and s.series_code=p_series_code
      and exists (select 1 from jsonb_array_elements(s.auto_pivots) pivot(value)
        where pivot.value->>'date'=target_date::text);
  update public.historical_indicator_ai_analysis a
    ...
    where a.case_code=p_case_code and a.series_code=p_series_code
      and exists (select 1 from jsonb_array_elements(a.pivots) pivot(value)
        where pivot.value->>'date'=target_date::text);
end;
$function$;

alter table public.historical_indicator_manual_pivots drop column is_deleted;
alter table public.historical_indicator_ai_scores drop column ai_pivots;
