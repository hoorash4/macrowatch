-- Keep an administrator's explicit key-off separate for each market index.
begin;
set local statement_timeout = '10s';

drop index if exists public.historical_manual_start_sp500_unique;
create unique index historical_manual_start_sp500_unique
  ...
  where not is_deleted and key_references->>'SP500' in ('START','PEAK','TROUGH');
drop index if exists public.historical_manual_start_nasdaq_unique;
create unique index historical_manual_start_nasdaq_unique
  ...
  where not is_deleted and key_references->>'NASDAQ_COMPOSITE' in ('START','PEAK','TROUGH');
drop index if exists public.historical_manual_start_kospi_unique;
create unique index historical_manual_start_kospi_unique
  ...
  where not is_deleted and key_references->>'KOSPI' in ('START','PEAK','TROUGH');
...
begin
  ...
  if p_pivot_date is null or p_pivot_value is null
    ...
    or p_relationship not in ('positive','inverse','unclear')
