...
begin
  ...
  if p_is_deleted then
    ...
    if p_key_reference in ('START', 'PEAK', 'TROUGH') then
      ...
      next_keys = next_keys || jsonb_build_object(p_index_code, p_key_reference);
    elsif p_key_reference is null or p_key_reference = 'AUTO' then
        ...
        for k, v in select key, value #>> '{}' from jsonb_each(next_keys) loop
          if v = 'VERIFIED' then
            if k = p_index_code then
              cleaned = cleaned || jsonb_build_object(k, false);
            end if;
...
-- Fix the stuck verified state on 1998-10-05 for US10Y
update public.historical_indicator_manual_pivots
set key_references = '{"SP500": false}'::jsonb
where case_code = 'asian_financial_crisis' and series_code = 'US10Y' and source_date = '1998-10-05';
