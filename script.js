...
function renderCombinedStressChart(monthlyRows, weeklyRows) {
  ...
  chart.innerHTML = `<div class="rounded-xl border border-slate-200 bg-slate-50 p-3"><svg class="w-full" style="height:${height}px" viewBox="0 0 ${width} ${height}" role="img" aria-label="US-MSI, MSI Lead 및 S&P 500 추이"><line x1="${padding.left}" x2="${padding.left}" y1="${padding.top}" y2="${height - padding.bottom}" stroke="#94a3b8"/><line x1="${width - padding.right}" x2="${width - padding.right}" y1="${padding.top}" y2="${height - padding.bottom}" stroke="#94a3b8"/>${grid}${yearGuides}${rightAxis}${monthlySegments}<path d="${path(weekly, 'lead_index', y, 'week')}" fill="none" stroke="#6d4b91" stroke-width="2.5"/><path d="${path(monthly, 'sp500_month_end_close', sy, 'month')}" fill="none" stroke="#285e8e" stroke-width="2.25"/>${monthlyDots}${years}</svg></div><div class="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs text-slate-400"><span class="inline-flex items-center gap-2"><i class="h-0.5 w-5 bg-amber-600"></i>US-MSI (월간)</span><span class="inline-flex items-center gap-2"><i class="h-0.5 w-5 border-t-2 border-dashed border-amber-600"></i>US-MSI 잠정치</span><span class="inline-flex items-center gap-2"><i class="h-0.5 w-5 bg-violet-800"></i>MSI Lead (주간)</span><span class="inline-flex items-center gap-2"><i class="h-0.5 w-5 bg-blue-800"></i>S&P 500 월말 종가</span></div>`;
}
...
function renderCreditStressMomentum(rows, monthlyRows = []) {
  ...
  const monthlyData = [...rows]
    .map((row) => ({ ...row, value: toCreditStressNumber(row.lead_momentum) }))
  ...
  if (!data.length) {
    chart.innerHTML = '<div class="flex min-h-40 items-center justify-center rounded-xl border border-dashed border-slate-700 bg-slate-950/30 p-5 text-sm text-slate-500">첫 산출 후 월간 스트레스 변화가 표시됩니다.</div>';
    return;
  }
  const width = 920;
  const height = 190;
  ...
  chart.innerHTML = `<div class="rounded-xl border border-slate-200 bg-slate-50 p-3"><svg class="w-full" style="height:${height}px" viewBox="0 0 ${width} ${height}" role="img" aria-label="미국 주간 스트레스 변화 추이">${grid}${yearGuides}${lines}${averageLines}</svg></div><div class="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-slate-400"><span class="inline-flex items-center gap-2"><i class="h-0.5 w-5 bg-red-900"></i>스트레스 확대</span><span class="inline-flex items-center gap-2"><i class="h-0.5 w-5 bg-blue-900"></i>스트레스 완화</span><span class="inline-flex items-center gap-2"><i class="h-0.5 w-5 bg-violet-800"></i>4주 평균</span></div>`;
}
