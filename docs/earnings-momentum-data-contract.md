...
최초 유니버스 등록 시 각 기업의 최근 5년 분기재무를 백필한다. 지수에 새로 진입한 기업도 교체 삭제하지 않고 기업 마스터와 소속 이력에 누적한 뒤 최근 5년을 백필한다.
...
재진입 시에는 새 기업을 만들지 않고 기존 기업에 새로운 소속 기간을 추가한다. 동시에 최근 5년의 예상 회계분기와 저장된 표준 분기를 비교하는 연속성 검사를 실행하고, 이탈 기간이나 자동수집 실패로 비어 있는 분기가 있으면 해당 분기만 자동 백필한다. 이미 완성된 분기는 다시 적재하지 않되 정정공시는 기존 정정 규칙에 따라 반영한다. 누락 보완이 끝나면 해당 기업을 다시 정기 수집 대상에 포함한다.
