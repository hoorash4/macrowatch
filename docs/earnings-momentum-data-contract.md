...
기업과 지수 소속도 분리한다. 기업은 `earnings_companies`에 한 번만 존재하고, 복수 지수 소속은 `earnings_index_memberships`에 각각 저장한다. 지수 이탈은 소속 기간만 종료하며 기업의 재무 수집 대상 상태를 종료하지 않는다.
...
한 번이라도 유니버스에 등록된 기업은 현재 지수에서 이탈해도 일일 공시 확인 대상에 계속 포함한다. 이탈 시 `earnings_index_memberships.effective_to`만 기록하고 기업·식별자·재무자료는 유지한다. 따라서 정상 운영 중에는 이탈 기간에도 분기재무가 연속해서 쌓인다.

재진입 시에는 새 기업을 만들지 않고 기존 기업에 새로운 소속 기간을 추가한다. 동시에 최근 5년의 예상 회계분기와 저장된 표준 분기를 비교하는 연속성 검사를 실행하고, 이탈 기간이나 자동수집 실패로 비어 있는 분기가 있으면 해당 분기만 자동 백필한다. 이미 완성된 분기는 다시 적재하지 않되 정정공시는 기존 정정 규칙에 따라 반영한다.

5년 원자료는 YoY와 QoQ 계절 기준을 계산하는 공통 입력이다. 화면 표시 범위와 원자료 보존 범위는 분리하며, 지수 이탈 기업의 과거 재무와 순위도 삭제하지 않는다. 과거 지수 집계와 순위는 각 분기의 `effective_from`·`effective_to` 소속 기간을 기준으로 계산하므로, 이탈 기간에 수집된 재무가 당시 지수 결과에 잘못 포함되지 않는다.
