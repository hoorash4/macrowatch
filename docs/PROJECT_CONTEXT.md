...
- 비교 지표는 하나의 통합 아코디언에서 관리하고 라디오 버튼으로 한 번에 하나만 차트에 표시한다. 선택 지표는 보라·마젠타 계열의 단일 강조색을 사용한다. 과거사례는 시장별 START/PEAK/TROUGH를 먼저 확정하고 각 기준점의 -3개월~+1개월 안에서만 기술적 피봇 후보를 찾는다. 후보의 진위는 화면용으로 불러온 넓은 이전·이후 전체 경로에서 확정 regime 전환과 기존 추세 재개 여부를 별도로 검증하며, 검증된 market-relevant pivot만 기준점에 연결한다. retrospective 경로는 직전 추세 진행폭, 되돌림 깊이, 관측 주기별 smoothing과 변동성 임계값, 원래 방향 복귀 및 새 극점 갱신을 함께 사용해 얕은 correction/rebound를 기존 추세에 통합한다. 방향 추세가 횡보로 끝날 때는 실제 극점을, 횡보에서 방향 추세가 시작될 때는 range의 구조적 이탈점을 경계로 사용한다. 의미 있는 피봇이 없는 기준점은 null로 유지한다. 시장 추세가 3개월 이하·6개월 이하·그보다 길 때 지표 regime 최소기간은 각각 1·2·3개월이다. 현재국면은 별도의 online 경로에서 과거 어느 시장에서든 의미 있었던 전체 지표를 감시한다. 시간 경과만으로 추세를 종료하거나 후보·확정으로 승격하지 않으며, 직전 추세 진행폭 대비 되돌림 깊이, 지표 변동성, 전고점·전저점 갱신, 횡보 범위의 구조적 이탈과 복귀 여부를 함께 판정한다. 최소 regime 기간은 구조적 전환이 확인된 뒤의 잡음 배제 하한으로만 사용하고, 얕은 correction/rebound는 기간과 관계없이 watch로 유지한다. 새 극점 갱신이나 기존 범위 복귀는 후보를 무효화하며, watch/candidate/confirmed 누적 기여도로 새 사이클 시작 가능성을 표시한다.
