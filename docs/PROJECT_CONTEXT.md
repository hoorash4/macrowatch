...
과거 국면 TROUGH 이후의 관계 적합성과 저장된 정·역관계의 색 판정은 해당 피봇 뒤 첫 변곡점까지만 비교한다. 다음 변곡점이 없으면 TROUGH의 24개월 버퍼 안에서 지표의 마지막 실제 관측일을 종료일로 쓴다. 지표 추세는 피봇일부터, 지수 추세는 TROUGH 기준일부터 출발하고, 두 시계열에 동일한 종료일을 적용한다. 완전한 수평 선분은 진입 추세를 이어받아 일수의 절반으로 계산한다.
