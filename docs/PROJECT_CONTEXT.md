...
경제지표 metadata는 `economic-series-catalog.js`가 코드·표시명·범주·주기·단위의 단일 기준점이다. Historical Insight의 지표 사용 가능 여부는 canonical `economic_chart_series_points`를 집계한 `economic_chart_series_coverage`의 실제 최초·최종 관측일로 판정한다. 추세 구조는 원래 관측 주기를 보존한 raw 값과 주기별 smoothing으로 판정하고, 차트 오버레이에만 표시 구간별 0~100 정규화를 사용한다. smoothing은 구조 판정에만 쓰며 최종 피봇 날짜와 값은 raw 시계열의 실제 극점 또는 실제 range 이탈점으로 되돌린다. `pivotDate`, `regimeBoundaryDate`, `confirmationDate`는 별도 필드다. 방향 추세에서 끝나는 전환은 실제 극점을, sideways에서 방향 추세로 나가는 전환은 구조적 breakout/breakdown 경계를 피봇으로 사용한다.

현재국면 분석은 완결된 과거사례의 retrospective 엔진과 물리적으로 분리된 online 상태 엔진으로 실행한다. 현재 사례에 이미 확정된 START/PEAK/TROUGH는 과거사례와 같은 기준점 중심 retrospective 방식으로 분석하고, 기준점이 없는 최신 구간은 `watch / candidate / structural_confirmed`로만 관리한다. `structural_confirmed`는 지표 구조가 확인됐다는 뜻이며 market-relevant 확정과 구분한다. 과거 사례에서 START/PEAK/TROUGH 중 한 번이라도 의미 있는 피봇이 있었던 지표 전체를 상시 목록으로 유지한다. 조정·반등 watch에서는 최근 구조 확정 기여도를 유지하고, 기존 추세 방향의 새 extreme이 나오면 후보와 예비 기여도를 회수한 뒤 실제 새 극점으로 교체한다. 주가 피봇 가능성은 이 기여도의 결정론적 집계이며 통계적 예측 확률이 아니다. 현재 진행 중인 사례가 없으면 이름은 `historical_current_settings`의 기본값인 `현재 국면 관찰 중`을 사용한다. 진행 중 사례 이름과 기본 이름은 관리자만 수정할 수 있다.
...
- 비교 지표는 하나의 통합 아코디언에서 관리하고 라디오 버튼으로 한 번에 하나만 차트에 표시한다. 선택 지표는 보라·마젠타 계열의 단일 강조색을 사용한다. 과거사례의 분석 단위는 `시장 기준점 × 지표`다. 시장별 START/PEAK/TROUGH를 먼저 확정하고 각 기준점의 -3개월~+1개월 안에서만 후보를 찾은 뒤, 후보보다 넓은 전체 전후 경로에서 독립적으로 structural pivot을 검증한다. 전체 피봇 목록을 기준점에 나눠 배정하지 않으며 같은 지표가 세 기준점 모두에서 의미 있으면 세 결과를 보존한다. retrospective 경로는 직전 추세 진행폭, 되돌림 깊이, 관측 주기별 변동성, 경로 효율, 기존 방향 복귀와 새 극점 갱신을 함께 사용해 correction/rebound를 통합한다. 시장 추세가 3개월 이하·6개월 이하·그보다 길 때의 1·2·3개월 값은 새 regime의 최소 유효성 검사에만 쓰고 segmentation, 최대 조정기간, 자동 확정에는 쓰지 않는다. 구조가 없거나 최소 유효기간을 충족하지 않으면 기준점 결과는 null이다. 구조 검증 뒤 기준점별 local 변화율을 관측 주기에 맞는 여러 lag에서 시장과 비교해 `positive / inverse / unclear`를 산출한다. 명확하고 부호가 안정된 positive/inverse에만 중앙 정책값 범위의 relationship bonus를 더한다. unclear는 피봇을 삭제하거나 기본 점수를 감점하지 않으며 최종 점수는 structural validity·timing·duration의 base score에 bonus를 더한다. 현재국면은 별도 online 경로에서 과거 어느 시장에서든 의미 있었던 전체 지표를 감시한다. 시간 경과만으로 추세를 종료하거나 후보·확정으로 승격하지 않으며, 직전 추세 진행폭 대비 되돌림 깊이, 지표 변동성, 전고점·전저점 갱신, 횡보 범위의 구조적 이탈과 복귀 여부를 함께 판정한다. 최소 regime 기간은 구조적 전환이 확인된 뒤의 잡음 배제 하한으로만 사용하고, 얕은 correction/rebound는 기간과 관계없이 watch로 유지한다. 새 극점 갱신이나 기존 범위 복귀는 후보를 무효화하고 기여도를 회수하며, 최신 구간은 watch/candidate/structural_confirmed 누적 기여도로 새 사이클 시작 가능성을 표시한다.
