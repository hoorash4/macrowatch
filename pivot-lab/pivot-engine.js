(() => {
  ...
  function buildRegimes(months,moves,states){
    ...
    for(let i=1;i<=states.length;i++){
      ...
      regimes.push({state,startMonth:startMove,endMonth:i});
  ...
  function detect(rows){
    ...
    for(let i=0;i<regimes.length-1;i++){
      ...
      if(!pivot)continue;
      candidates.push({type:pivot.type,index:pivot.point.index,date:pivot.point.time,value:pivot.point.value,source:'monthly-regime-boundary',reason:`${left.state}-to-${right.state}`});
    }
    ...
    return {
      ...
      diagnostics:{engineVersion:'monthly-regime-v1',major:pivots.length,sidewaysZones,deviation:0,monthly:months.length,raw:rows.length,regimes:regimes.length}
    };
  }
