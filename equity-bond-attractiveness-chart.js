(() => {
  ...
  function render() {
    ...
    frame.addEventListener('pointermove', event => {
      ...
      value.setAttribute('x', x); value.textContent = rows.map(row => `${LABELS[row.country]} ${Number(row.score).toFixed(1)} (이익수익률 ${Number(row.earnings_yield_pct).toFixed(2)}% · 10년물 ${Number(row.sovereign_yield_pct).toFixed(2)}% · 이익모멘텀 ${Number(row.earnings_momentum_pct).toFixed(1)}% · 상대수익률 ${Number(row.relative_return_13w_pct).toFixed(1)}%)`).join('  |  '); value.setAttribute('visibility', 'visible');
  ...
  async function load({ supabaseClient }) {
    ...
    const { data, error } = await utils.loadAllRows((from, to) => supabaseClient.from('equity_bond_attractiveness_weekly').select('country,observation_date,score,earnings_yield_pct,sovereign_yield_pct,yield_gap_pct,earnings_momentum_pct,relative_return_13w_pct').eq('method_version', 'equity-bond-attractiveness-v1').order('observation_date').order('country').range(from, to));
