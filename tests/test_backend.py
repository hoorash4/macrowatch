...
class SourceContractTests(unittest.TestCase):
    ...
    def test_sector_flow_stores_open_close_and_server_rankings(self):
        ...
        self.assertIn("KIS_REQUEST_INTERVAL_MS", pipeline)
        self.assertIn("KIS_RATE_LIMIT_RETRY_DELAYS_MS", pipeline)
        self.assertIn("createKisRequestRunner", pipeline)
        self.assertIn('message.includes("초당 거래건수를 초과")', pipeline)
    ...
    def test_fomc_pipeline_normalizes_ai_output_before_storage(self) -> None:
        ...
        self.assertIn("JSON.parse(storedPrompt)", pipeline)
