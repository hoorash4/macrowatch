...
from earnings.sec_parser import METRIC_ALIASES, canonical_sec_quarters  # noqa: E402
...
class MirrorSession:
    ...
    def get(self, _url, *, params, **_kwargs):
        ...
        if "FROM xbrl_tags" in query:
            ...
            rows = [{"tag_id": index + 1, "tag": tag} for index, tag in enumerate(tags)]
...
class SecEarningsTests(unittest.TestCase):
    def test_mirror_rebuilds_the_official_company_facts_envelope(self):
        ...
        fact = payload["facts"]["us-gaap"]["OperatingIncomeLoss"]["units"]["USD"][0]
    ...
    def test_losses_are_kept_but_an_all_zero_profit_pair_is_a_gap(self):
