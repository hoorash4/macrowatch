...
test('UI uses one radio-selected magenta indicator without dimming other series',()=>{
  ...
  assert.match(css,/\.historical-indicator-result-grid strong \{[^}]*font-size: 14px/);assert.match(css,/\.historical-indicator-result-grid p \{[^}]*font-size: 13px/);assert.match(controller,/card\.classList\.toggle\('is-empty',!hasPivot\)/);
  ...
  assert.match(controller,/기준점 \$\{dates\[type\]\}/);assert.match(controller,/피봇점 \$\{score\.pivotDate\}/);
