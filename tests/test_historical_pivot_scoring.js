...
test('a leading pivot starts indicator trend before the index reference date',()=>{
  const rows=[{time:'2021-12-01',value:0},{time:'2022-01-01',value:10},{time:'2022-02-01',value:10}];
  const at=(source,date)=>source.find(row=>row.time===date)?.value;
  const input={pivots:[],fromDate:'2022-01-01',toDate:'2022-02-01',rows,valueAtDate:at,
    expectedDirection:1,factor:1,startingPivot:pivot('2021-12-01')};
  assert.deepEqual({...scoring.relationshipScore(input)},{relationship:'positive',score:100});
  assert.equal(scoring.relationshipScore({...input,startingPivot:null}).score,0);
});
...
test('saved relationship color uses only the first pivot segment within its reference period',async()=>{
  const {mergedPivots}=await storedScoring;
  const cycle={startDate:'2022-01-01',peakDate:'2022-07-01',troughDate:'2023-01-01'};
  const manual=[{source_date:'2022-01-01',pivot_date:'2022-01-01',pivot_value:30,
    relationship:'inverse',key_references:{}}];
  const observations=[{observation_date:'2022-01-01',value:30},{observation_date:'2022-02-01',value:20},
    {observation_date:'2022-03-01',value:40},{observation_date:'2022-07-01',value:50},
    {observation_date:'2023-01-01',value:10}];
  const automatic=[{pivot_order:0,pivot_date:'2022-02-01',pivot_value:20},
    {pivot_order:1,pivot_date:'2022-03-01',pivot_value:40}];
  const pivots=mergedPivots({automatic,manual,cycle,indexCode:'SP500',observations});
  assert.equal(pivots[0].markerStatus,'confirmed');
  assert.equal(pivots[0].selectedReferences[0].type,'START');

  const peakCycle={startDate:'2022-01-01',peakDate:'2022-07-01',troughDate:'2023-01-01'};
  const peakManual=[{source_date:'2022-07-01',pivot_date:'2022-07-01',pivot_value:30,
    relationship:'positive',key_references:{}}];
  const peakObservations=[{observation_date:'2022-07-01',value:30},{observation_date:'2023-01-01',value:20},
    {observation_date:'2023-02-01',value:40}];
  const laterPivot=[{pivot_order:0,pivot_date:'2023-02-01',pivot_value:40}];
  const peakPivots=mergedPivots({automatic:laterPivot,manual:peakManual,cycle:peakCycle,
    indexCode:'SP500',observations:peakObservations});
  assert.equal(peakPivots[0].markerStatus,'confirmed');
});
...
test('stored relationship includes a leading pivot while the index benchmark starts at its reference',async()=>{
  const {mergedPivots,scoreReferences}=await storedScoring;
  const cycle={startDate:'2022-01-01',peakDate:'2022-02-01',troughDate:'2022-03-01'};
  const rows=[{observation_date:'2021-12-01',value:0},{observation_date:'2022-01-01',value:10},
    {observation_date:'2022-02-01',value:10}];
  const manual=[{source_date:'2021-12-01',pivot_date:'2021-12-01',pivot_value:0,
    relationship:null,key_references:{SP500:'START'},is_deleted:false}];
  const pivots=mergedPivots({manual,cycle,indexCode:'SP500'});
  const score=scoreReferences({pivots,rows,cycle}).START;
  assert.equal(score.relationship,'positive');
  assert.equal(score.relationshipSuitabilityScore,100);
});
