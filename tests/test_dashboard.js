...
test('모바일 최근 뉴스 막대는 화면 폭을 사용하고 날짜를 막대 왼쪽 위에 둔다', () => {
  ...
  assert.match(styles, /@media \(max-width:768px\)[\s\S]*?#news-sentiment-chart \.news-sentiment-horizontal-bar\s*\{[\s\S]*?width: 100%;[\s\S]*?height: 4rem;[\s\S]*?flex: none;/);
});
...
test('주도섹터는 이번 주와 과거 4주를 표시하고 한 주를 변동 기준으로 조회한다', () => {
  ...
  assert.match(styles, /sector-flow-week-current \.sector-flow-returns \{[\s\S]*padding:\.28rem \.35rem;[\s\S]*background:rgba\(200,107,32,\.055\)/);
...
test('모바일 카드 폭은 화면 안에 고정되고 주도섹터는 주차별로 스와이프한다', () => {
  ...
  assert.match(charts, /indicator\.setAttribute\('aria-label', `주차 \$\{activeIndex \+ 1\}\/\$\{cards\.length\}`\)/);
