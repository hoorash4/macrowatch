...
class PolicyRateSourceTests(unittest.TestCase):
    ...
    def test_us_range_rows_store_both_bounds_and_midpoint(self, observations):
        ...
        chart_rows = us_policy_change_chart_rows(rows)
        self.assertEqual([row["observation_date"] for row in chart_rows], ["2026-01-01", "2026-01-03"])
        self.assertEqual([row["value"] for row in chart_rows], [4.375, 4.125])
