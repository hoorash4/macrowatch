...
class EconomicChartFeatureTests(unittest.TestCase):
    ...
    def test_horizontal_line_has_one_combined_axis_marker_and_flat_bell(self):
        ...
        self.assertIn('if(from<-to){from=-w/2;to=w/2;}', script)
        self.assertIn('maxW=Math.max(MIN_VISIBLE_BARS,anchor/MIN_DATA_SCREEN_RATIO)', script)
        self.assertIn('next={from:anchor-nextW,to:anchor}', script)
    ...
    def test_economic_chart_collection_is_automatic_only(self):
        ...
        self.assertIn('check_collected_series_alerts(db, tracked_codes)', automatic)
    ...
    def test_us_policy_rate_is_stored_as_decision_events_not_daily_rows(self):
        ...
        self.assertIn('DERIVED:central_bank_policy_events', source)
    ...
    def test_core_series_include_fred_ecos_pykrx_and_derived_spreads(self):
        ...
        for token in ('DGS2', 'DGS10', 'BAMLH0A0HYM2', 'NFCICREDIT', 'BAMLEMCBPIOAS', 'DCOILWTICO', 'WEI', 'RRPONTSYD', 'WTREGEN', 'EMRATIO'):
            self.assertIn(token, pipeline)
        foreign_flow = (ROOT / 'supabase/functions/korea-foreign-flow/index.ts').read_text(encoding='utf-8')
        self.assertIn('731Y001', foreign_flow)
        self.assertIn('0000001', foreign_flow)
