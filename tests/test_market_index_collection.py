...
class MarketIndexCollectionTests(unittest.TestCase):
    ...
    def test_automatic_keeps_yahoo_rows_when_fred_verification_times_out(self):
        import requests
        ...
        def sample(code, _start, _end, **kwargs):
            return [row(code, "2026-09-11"), row(code, "2026-09-14")]

        with (
            patch("signals.market_index_collection.fetch_index_candles", side_effect=sample),
            patch(
                "signals.market_index_collection._fetch_fred_close",
                side_effect=requests.ReadTimeout("FRED timeout"),
            ),
        ):
            stored = automatic(db, today=today)

        self.assertEqual({code: 2 for code in INDEX_CODES}, stored)
        us_batches = [rows for _, rows, _ in db.upserts if rows[0]["index_code"] != "KOSPI"]
        self.assertEqual(2, len(us_batches))
        for rows in us_batches:
