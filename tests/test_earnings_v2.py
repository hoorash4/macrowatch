...
class EarningsV2BoundaryTests(unittest.TestCase):
    ...
    def test_kis_top_line_adapter_is_protected_and_uses_standard_sales_account(self):
        ...
        self.assertIn("isTrustedServerRequest(request, supabaseUrl)", adapter)
        self.assertIn('request.headers.get("apikey")', auth)
        self.assertIn('Deno.env.get("SUPABASE_SECRET_KEYS")', auth)
        self.assertIn('verifier.rpc("earnings_v2_get_company_quarters_many"', auth)
        self.assertIn("p_company_ids: []", auth)
