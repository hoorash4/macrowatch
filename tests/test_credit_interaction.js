...
test('credit chart reuses earnings frame and exposes three values without inventing missing bankruptcy data',()=>{
...
test('all-null series and a single observation retain finite chart coordinates',()=>{
  ...
  h.resize();
  assert.doesNotMatch(h.chart.innerHTML,/NaN|Infinity/);
  assert.doesNotMatch(h.node('[data-credit-left-axis]').innerHTML,/NaN|Infinity/);
});
