...
class CensusRetailSalesTests(unittest.TestCase):
    ...
    def test_fetch_uses_official_marts_codes_and_api_key(self):
        ...
        self.assertEqual(result, {date(2026, 7, 1): 700123.0})
        kwargs = session.get.call_args.kwargs
        self.assertEqual(kwargs["params"]["category_code"], "44X72")
        self.assertEqual(kwargs["params"]["data_type_code"], "SM")
        self.assertEqual(kwargs["params"]["time"], "2026")
        self.assertEqual(kwargs["params"]["key"], "secret")
        self.assertNotIn("seasonally_adj", kwargs["params"])

    def test_chart_rows_match_existing_economic_chart_schema(self):
        rows = census.chart_rows({date(2026, 7, 1): 700123.0})
    ...
    def test_census_only_backfill_does_not_invoke_other_sources(self):
        ...
        with (
            ...
            patch.object(backfill, "fetch_census_retail_sales", return_value={date(2020, 1, 1): 500000.0}),
    ...
    def test_workflows_and_frontend_include_census_retail_contract(self):
