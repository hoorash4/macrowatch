(() => {
  ...
  function create(){
    ...
    function reconcile(items,autoSelect=false){available=items.map(item=>item.meta.code);checked=new Set([...checked].filter(code=>available.includes(code)));if(autoSelect&&!checked.size&&available.length)checked.add(available[0]);if(!active||!checked.has(active))active=available.find(code=>checked.has(code))||null;return snapshot();}
