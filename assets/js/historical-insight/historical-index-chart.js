(() => {
  ...
  class PivotLineRenderer {
    ...
    draw(target) {
      target.useBitmapCoordinateSpace((scope) => {
        ...
        const px = Math.round(x * h);
        if (px < 0 || px > scope.bitmapSize.width) {
          return;
        }
        const ctx = scope.context;
        ctx.save();
        ctx.strokeStyle = this.view.color;
        ctx.lineWidth = Math.max(1, h);
        ctx.setLineDash([3 * h, 3 * h]);
        ctx.beginPath();
        ctx.moveTo(px, 0);
  ...
  class PivotLineView {
    constructor(chart, time, color, timelineRows = []) {
      this.chart = chart;
      this.time = time;
      this.color = color;
      this.timelineRows = timelineRows;
      this.x = null;
      this.rendererInstance = new PivotLineRenderer(this);
    }
      ...
      if (coord === null && Array.isArray(this.timelineRows) && this.timelineRows.length) {
        ...
        if (Number.isFinite(targetMs)) {
          ...
          for (let i = 0; i < this.timelineRows.length; i++) {
            ...
            if (diff < minDiff) {
              minDiff = diff;
              closest = this.timelineRows[i].time;
            }
          }
          if (closest && minDiff <= 7 * 86400000) {
            coord = this.chart.timeScale().timeToCoordinate(closest);
          }
        }
      }
      this.x = coord;
  ...
  class PivotTimeAxisRenderer {
    ...
    draw(target) {
      target.useBitmapCoordinateSpace((scope) => {
        ...
        const ctx = scope.context;
        const labelHeight = 18 * v;
        const labelGap = 2 * v;
        const labelTop = scope.bitmapSize.height - (this.view.lane + 1) * labelHeight - this.view.lane * labelGap - 3 * v;
        const padX = 5 * h;

        ctx.save();
        ctx.font = `${10 * v}px Pretendard, sans-serif`;
        ctx.textBaseline = 'middle';

        const labelWidth = ctx.measureText(this.view.date).width + padX * 2;
        const labelX = Math.max(2 * h, Math.min(px - labelWidth / 2, scope.bitmapSize.width - labelWidth - 2 * h));
        this.view.bounds = {
          left: labelX / h,
          right: (labelX + labelWidth) / h,
          top: labelTop / v,
          bottom: (labelTop + labelHeight) / v,
        };
        ...
        ctx.beginPath();
        ctx.moveTo(px, 0);
        ctx.lineTo(px,labelTop);
        ctx.stroke();
        ctx.setLineDash([]);

        ctx.fillStyle = this.view.color;
        ctx.fillRect(labelX, labelTop, labelWidth, labelHeight);
        ctx.fillStyle = this.view.textColor;
        ctx.fillText(this.view.date, labelX + padX, labelTop + labelHeight / 2);
        ctx.restore();
      });
    }
  }
  ...
  class PivotTimeAxisPaneView {
    constructor(paneView, date, color, textColor, lane = 0) {
      ...
      this.lane = lane;
      this.bounds = null;
      this.rendererInstance = new PivotTimeAxisRenderer(this);
    }
  ...
  class PivotLinePrimitive {
    constructor(chart, time, color, textColor = '#fff', lane = 0, timelineRows = []) {
      this.view = new PivotLineView(chart, time, color, timelineRows);
      this.timeAxisPaneView = new PivotTimeAxisPaneView(this.view, time, color, textColor, lane);
    }
  ...
  function create(host) {
    ...
    const referenceOnlyStyle = () => ({
      color: getComputedStyle(host).getPropertyValue('--historical-near-miss-color').trim() || '#cbd5e1',
      textColor: getComputedStyle(host).getPropertyValue('--historical-near-miss-text').trim() || '#475569',
    });

    const nearMissStyle=()=>({
      color: getComputedStyle(host).getPropertyValue('--historical-quasi-core-color').trim()||'#5a92bb',
      textColor: '#fff'
    });

    const verifiedStyle=()=>({
      color: getComputedStyle(host).getPropertyValue('--historical-verified-color').trim()||(document.documentElement.dataset.theme==='dark'?'#475569':'#64748b'),
      textColor: '#fff'
    });
    ...
    function pointValueAt(rows, time, key = 'value', chartX = null) {
      ...
      if (!target || !rows.length || target < rows[0].time || target > rows.at(-1).time) {
        return null;
      }
      let left = 0;
      ...
      if (!Number.isFinite(from) || !Number.isFinite(to) || !Number.isFinite(at) || to === from) {
        return null;
      }
    ...
    function hideCrosshairOverlay() {
      ...
      for (const marker of crosshairMarkers.values()) {
        marker.remove();
      }
    ...
    function updateCrosshair(param) {
      if (!param?.point || !param.time || !series || !data.length) {
        return hideCrosshairOverlay();
      }
      ...
      const x = Math.round(chart.priceScale('left').width() + chartX);
      const indicators = [];
      ...
      for (const [code, item] of indicatorSeries) {
        ...
        if (Number.isFinite(plotValue) && Number.isFinite(value)) {
          indicators.push({
            code,
            value,
            plotValue,
    ...
    function onChartClick(param) {
      if (!indicatorPointClick || !param?.point || !param.time || !chart) {
        return;
      }
      const date = isoDate(param.time);
      const x = chart.priceScale('left').width() + param.point.x;
      for (const [code, item] of indicatorSeries) {
        const marker = crosshairMarkers.get(code);
        if (!marker || marker.dataset.date !== date) {
          continue;
        }
        if (Math.hypot(x - Number.parseFloat(marker.style.left), param.point.y - Number.parseFloat(marker.style.top)) > 6) {
          continue;
        }
        const value = pointValueAt(item.rows, param.time, 'rawValue', param.point.x);
        if (Number.isFinite(value)) {
          indicatorPointClick({ code, date, value });
        }
        break;
      }
    }
    ...
    function onTimeAxisClick(event) {
      if (!indicatorPointClick || !chart || event.button !== 0) {
        return;
      }
      ...
      for (const [code, item] of indicatorSeries) {
        for (const primitive of [...item.primitives].reverse()) {
          ...
          const date = primitive.view.time;
          const value = pointValueAt(item.rows, date, 'rawValue', timeScale.timeToCoordinate(date));
          if (Number.isFinite(value)) {
            indicatorPointClick({ code, date, value });
          }
          return;
    ...
    return Object.freeze({
      ...
      setCycle(points) {
        ...
        series.setMarkers(points.map((point) => ({
          time: point.row.time,
          ...style[point.type],
      ...
      setIndicators(items) {
        ...
        for (const entry of indicatorSeries.values()) {
          for (const primitive of entry.primitives) {
            entry.series.detachPrimitive(primitive);
          }
        ...
        items.forEach((item) => {
          ...
          const pivotPriority = (result) => {
            if (result.markerStatus === 'reference_only') return 0;
            if (result.markerStatus === 'verified') return 1;
            if (['near_miss', 'overridden_key', 'manual_standard'].includes(result.markerStatus)) return 2;
            return 3;
          };

          const orderedPivots = [...(item.displayPivots||item.results)].sort((a, b) => pivotPriority(a) - pivotPriority(b));
          const primitives = orderedPivots.map((result) => {
            const style = pivotStyle(result, color);
            return new PivotLinePrimitive(chart, result.pivotDate, style.color, style.textColor, 0, data);
          });

          for (const primitive of primitives) {
            line.attachPrimitive(primitive);
          }
      ...
      indicatorColors() {
        return new Map([...indicatorSeries].map(([code, item]) => [code, item.color]));
      },

      setIndicatorPointClick(handler) {
        indicatorPointClick = typeof handler === 'function' ? handler : null;
      },

      focus(from, to, markerInset = 0.12) {
        if (!chart || !from || !to || !data.length) {
          return;
        }
        const startIndex = data.findIndex((row) => row.time >= from);
        const endIndex = data.findLastIndex((row) => row.time <= to);
