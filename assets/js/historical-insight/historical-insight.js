(() => {
  ...
  function renderStoredPivotScores(item,context){
    ...
    referenceOrder.forEach((type,index)=>{
      ...
      const label=card.querySelector('strong'),body=card.querySelector('p');
      label.textContent=`${type} · ${pivot?(offset===0?'기준점 당일':`기준점 ${Math.abs(offset)}일 ${offset<0?'전':'후'}`):'기준점 피봇 없음'} · ${relationshipLabel(relation.relationship)}`;
      ...
      const reasonBox=card.querySelector('.historical-pivot-reason');
      if(pivot){
        if(reasonBox)reasonBox.querySelector('p').textContent=pivotReasonFor(item,pivot)||'근거 미저장';
        else{const box=document.createElement('div'),reasonLabel=document.createElement('b'),reasonText=document.createElement('p');box.className='historical-pivot-reason';reasonLabel.textContent='피봇 판정 근거';reasonText.textContent=pivotReasonFor(item,pivot)||'근거 미저장';box.append(reasonLabel,reasonText);card.append(box);}
      }else reasonBox?.remove();
    });
  }
