(() => {
  ...
  const MANUAL_PIVOT_REASONS=Object.freeze([
    '장기 상승을 마친 뒤 하락 흐름이 이어지기 시작했습니다.',
    ...
    '오랫동안 이어진 하락이 멈추고 저점권에서 새로운 흐름을 준비하기 시작했습니다.',
  ...
  function mergeManualPivots(item,cycle){
    ...
    return classified.map(pivot=>{
      ...
      return Object.freeze({...pivot,selectedReferences:Object.freeze(selectedReferences),markerStatus:pivot.isManual&&!pivot.keyReference?'manual_standard':overridden?'overridden_key':selectedReferences.length?'confirmed':pivot.markerStatus});
    });
  }
