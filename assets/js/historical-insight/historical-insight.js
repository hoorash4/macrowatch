(() => {
  ...
  function displayItem(item,context){let displayPivots=context.mode==='history'&&item.storedPivots?.length?classifyStoredPivots(item.storedPivots,context.cycle):[...(item.results||[]),...(item.nearMissPivots||[]),...(item.manualDisplayPivots||[])];if(context.mode==='current'){const latest=item.evidence?.pending?{pivotDate:item.evidence.pending.candidateDate,regimeBoundaryDate:item.evidence.pending.regimeBoundaryDate,referenceType:'CURRENT_STRUCTURAL',markerStatus:item.evidence.status}:item.evidence?.result||null;displayPivots=[...(item.confirmedReferences||[]),...(latest?[latest]:[])];}displayPivots=[...new Map(displayPivots.map(pivot=>[`${pivot.markerStatus||''}:${pivot.referenceType||''}:${pivot.pivotDate}`,pivot])).values()];return{...item,displayRows:indicatorAnalysis.normalizeForDisplay(item.rows,context.displayRange.from,context.displayRange.to),displayPivots};}
