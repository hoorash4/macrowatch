(() => {
  ...
  const PROFILE = chartUtils.chartProfile({
    xAxisMode: 'zero',
    cursorSeries: Object.freeze([{ key: 'fiveDayAverage', label: '이머징 자금여건' }]),
  });
  ...
  function withFiveDayAverage(rows) {
    return rows.map((row, index) => {
      if (index < 4) {
        return { ...row, fiveDayAverage: null };
      }
      const values = rows.slice(index - 4, index + 1).map((item) => Number(item.capacity_index));
      const sum = values.reduce((acc, value) => acc + value, 0);
      return { ...row, fiveDayAverage: sum / values.length };
    });
  }

  function verticalScale(points) {
    const values = points
      .flatMap((point) => [point.value, point.fiveDayAverage])
  ...
  function render(container, rows, selectedYears) {
    const points = withFiveDayAverage(rows)
      .map((row) => ({
        ...row,
        timestamp: Date.parse(`${row.observation_date}T00:00:00Z`),
        value: Number(row.capacity_index),
      }))
    ...
    const pathFor = (key, sourceDomain, sourcePoints = points) => {
      const filtered = sourcePoints
    ...
    const grids = tickSlots.map((index) => {
      ...
      return `<line data-em-capacity-y-grid="${index}" x1="${PADDING.left}" y1="${y}" x2="${timelineWidth - PADDING.right}" y2="${y}" class="policy-expectation-y-grid${zeroClass}"${visibility}/>`;
    }).join('');
    ...
    const labels = tickSlots.map((index) => {
      ...
      return `<line data-em-capacity-y-tick="${index}" x1="${Y_AXIS_WIDTH - 5}" y1="${y}" x2="${Y_AXIS_WIDTH}" y2="${y}" class="policy-expectation-y-tick"${visibility}/><text data-em-capacity-y-index="${index}" x="${Y_AXIS_WIDTH - 9}" y="${y + 3}" text-anchor="end" class="policy-expectation-y-label"${visibility}>${formatted}</text>`;
    }).join('');
    ...
    const { frame, svg } = chartUtils.mountChartFrame({
      ...
      ariaLabel: '이머징 자금 유입 여건',
      plotMarkup: `
        <svg class="policy-expectation-chart-svg" style="width:${timelineWidth}px" viewBox="0 0 ${timelineWidth} ${HEIGHT}" role="img" aria-label="0선을 중심으로 표시한 이머징 자금 유입 여건">
          <defs>
            <linearGradient id="em-capacity-line-gradient" gradientUnits="userSpaceOnUse" x1="0" y1="${PADDING.top}" x2="0" y2="${HEIGHT - PADDING.bottom}">
          ...
          <path d="${pathFor('value', initialScale)}" class="policy-expectation-line policy-expectation-line--raw" style="stroke:url(#em-capacity-line-gradient)"/>
          <path d="${pathFor('fiveDayAverage', initialScale, confirmedPoints)}" class="policy-expectation-line policy-expectation-line--average" style="stroke:url(#em-capacity-line-gradient)"/>
          <path d="${pathFor('fiveDayAverage', initialScale, provisionalPoints)}" class="policy-expectation-line policy-expectation-line--average em-capacity-line--provisional" style="stroke:#6b7280"/>
          <line data-em-capacity-cursor x1="0" y1="${PADDING.top}" x2="0" y2="${HEIGHT - PADDING.bottom}" class="policy-expectation-cursor"/>
          <text data-em-capacity-value x="0" y="16" text-anchor="middle" class="analysis-chart-cursor-text analysis-chart-cursor-value" visibility="hidden"></text>
    ...
    const updateVisibleScale = () => {
      ...
      yLabels.forEach((label) => {
        const index = Number(label.dataset.emCapacityYIndex);
        const value = current.ticks[index];
        const tick = container.querySelector(`[data-em-capacity-y-tick="${index}"]`);
    ...
    frame.addEventListener('pointermove', (event) => {
      ...
      detail.textContent = formatMonthDay(nearest.observation_date);
  ...
  async function load({ supabaseClient }) {
    ...
    const { data, error } = await supabaseClient
      .from('em_capital_capacity_daily')
      .select('observation_date,capacity_index,is_provisional')
    ...
    if (controls && controls.dataset.bound !== 'true') {
      ...
      controls.addEventListener('click', (event) => {
        ...
        controls.querySelectorAll('[data-em-capacity-range]').forEach((item) => {
  ...
  window.addEventListener('macrowatch:dashboard-view-changed', ({ detail }) => {
    if (detail?.view !== 'stress' || detail?.stressMarket !== 'em') {
      return;
    }
