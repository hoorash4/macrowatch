(() => {
  ...
  function renderLegend() {
    ...
    legend.innerHTML = [
      ...
      chartUtils.legendItem('미국 10년물 금리', { stroke: COLORS.treasury, width: 1.75 }),
  ...
  function render() {
    ...
    const monthly = state.monthly, policy = state.policy;
    ...
    const allDates = [...monthly.map(row => timestamp(row.month)), ...policy.map(row => timestamp(row.observed_on))].filter(Number.isFinite);
    ...
    const x = row => padding.left + (timestamp(row.observed_on || row.month) - first) / Math.max(1, last - first) * (width - padding.left - padding.right);
    const values = [
      ...monthly.flatMap(row => [Number(row.headline_yoy_pct), Number(row.core_yoy_pct)]),
      ...policy.flatMap(row => [Number(row.target_upper_pct), Number(row.treasury_10y_pct)]),
    ...
    const path = (rows, key, yFor = y) => chartUtils.monotoneSeriesPath(rows, row => x(row), row => yFor(Number(row[key])));
    ...
    const guides = years.map(year => {
      ...
      return point ? `<line x1="${x(point)}" x2="${x(point)}" y1="${padding.top}" y2="${height - padding.bottom}" stroke="#edf0f4"/><text x="${x(point)}" y="${height - 14}" text-anchor="middle" fill="#64748b" font-size="10">${year}</text>` : '';
    }).join('');
    host.innerHTML = `<svg class="w-full" style="height:${height}px" viewBox="0 0 ${width} ${height}" role="img" aria-label="헤드라인과 코어 통합물가지수, 기준금리와 미국 10년물 금리 추이">${grid}${guides}<path d="${path(finalRows, 'headline_yoy_pct')}" fill="none" stroke="${COLORS.headline}" stroke-width="3" stroke-linecap="round"/><path d="${path(bridgeRows, 'headline_yoy_pct')}" fill="none" stroke="${COLORS.headline}" stroke-width="3" stroke-dasharray="5 4" stroke-linecap="round"/><path d="${path(finalRows, 'core_yoy_pct')}" fill="none" stroke="${COLORS.core}" stroke-width="3" stroke-linecap="round"/><path d="${path(bridgeRows, 'core_yoy_pct')}" fill="none" stroke="${COLORS.core}" stroke-width="3" stroke-dasharray="5 4" stroke-linecap="round"/><path d="${stepPath(policy, x, y, 'target_upper_pct')}" fill="none" stroke="${COLORS.policy}" stroke-width="2.25"/><path d="${path(policy, 'treasury_10y_pct')}" fill="none" stroke="${COLORS.treasury}" stroke-width="1.75" stroke-linecap="round"/><rect data-inflation-hit x="${padding.left}" y="${padding.top}" width="${width - padding.left - padding.right}" height="${height - padding.top - padding.bottom}" fill="transparent"/><line data-inflation-cursor x1="0" x2="0" y1="${padding.top}" y2="${height - padding.bottom}" class="policy-expectation-cursor"/><text data-inflation-value x="0" y="16" text-anchor="middle" fill="#334155" font-size="10" font-weight="700" visibility="hidden"></text><text data-inflation-date x="0" y="${height - padding.bottom + 14}" text-anchor="middle" class="policy-expectation-cursor-detail"></text></svg>`;
    chartUtils.scrollableSvg(host.querySelector('svg'), width, baseWidth, {
      ...
      axes: [{ side: 'left', y, points: [
        ...monthly.flatMap(row => [{ x: x(row), value: Number(row.headline_yoy_pct) }, { x: x(row), value: Number(row.core_yoy_pct) }]),
        ...policy.flatMap(row => [{ x: x(row), value: Number(row.target_upper_pct) }, { x: x(row), value: Number(row.treasury_10y_pct) }]),
    ...
    chartUtils.scrollableSvg(realHost.querySelector('svg'), width, baseWidth, {
      ...
      axes: [{ side: 'left', y: realY, points: monthly.flatMap(row => [{ x: x(row), value: Number(row.headline_real_rate_pct) }, { x: x(row), value: Number(row.core_real_rate_pct) }]), selector: `path[stroke="${COLORS.headline}"],path[stroke="${COLORS.core}"]`, format: value => `${value.toFixed(1)}%` }],
    });
    ...
    const latestPolicyFor = row => policy.filter(point => timestamp(point.observed_on) <= monthEndTimestamp(row.month)).at(-1) || {};
    const showMonth = nearest => {
      const cursorX = x(nearest), labelX = Math.max(190, Math.min(width - 190, cursorX));
      ...
      mainValue.textContent = `헤드라인 ${formatValue(nearest.headline_yoy_pct)} · 코어 ${formatValue(nearest.core_yoy_pct)} (${status}) · 기준 ${formatValue(nearest.policy_rate_upper_pct)} · 10년 ${formatValue(policyPoint.treasury_10y_pct)}`;
      ...
      [host.querySelector('[data-inflation-date]'), realHost.querySelector('[data-inflation-real-date]')].forEach(label => {
        label.setAttribute('x', cursorX); label.textContent = formatCursorMonth(nearest.month); label.classList.add('is-visible');
      });
    };
    const clearMonth = () => {
    ...
    const attachPointer = (frame, svg) => frame?.addEventListener('pointermove', event => {
      ...
      showMonth(monthly.reduce((closest, row) => Math.abs(x(row) - pointerX) < Math.abs(x(closest) - pointerX) ? row : closest));
    });
