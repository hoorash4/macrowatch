(() => {
  ...
  function renderLegend() {
    ...
    if (legend) legend.innerHTML = [
      ...
      utils.legendItem('중소기업 업황전망 SBHI', { stroke: COLORS.headline, width: utils.lineWidths.comparison }),
    ].join('');
  }
  ...
  function render() {
    ...
    host.innerHTML = `<svg class="w-full" style="height:${HEIGHT}px" viewBox="0 0 ${width} ${HEIGHT}" role="img" aria-label="한국 중소기업 위험지수와 중소기업 업황전망 SBHI 월별 추이">${grid}${rightAxis}${guides}<path data-korea-small-business-risk-line d="${riskPath}" fill="none" stroke="${COLORS.risk}" stroke-width="${utils.lineWidths.primary}" stroke-linecap="round"/><path data-small-business-headline-line d="${headlinePath}" fill="none" stroke="${COLORS.headline}" stroke-width="${utils.lineWidths.comparison}" stroke-linecap="round"/><rect data-korea-small-business-risk-hit x="${PADDING.left}" y="${PADDING.top}" width="${width - PADDING.left - PADDING.right}" height="${HEIGHT - PADDING.top - PADDING.bottom}" fill="transparent"/><line data-korea-small-business-risk-cursor x1="0" x2="0" y1="${PADDING.top}" y2="${HEIGHT - PADDING.bottom}" class="policy-expectation-cursor"/><text data-korea-small-business-risk-value x="0" y="16" text-anchor="middle" fill="#334155" font-size="10" font-weight="700" visibility="hidden"></text><text data-korea-small-business-risk-date x="0" y="${HEIGHT - PADDING.bottom + 14}" text-anchor="middle" class="policy-expectation-cursor-detail"></text></svg>`;
    ...
    frame?.addEventListener('pointermove', event => {
      ...
      if (Number.isFinite(Number(nearest.headline_outlook_sbhi))) labels.push(`업황 ${Number(nearest.headline_outlook_sbhi).toFixed(1)}`);
