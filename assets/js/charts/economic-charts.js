...
const SERIES=[
 ...
 {code:'KR_EXPORT_DAILY_AVG',title:'한국 10일구간 일평균 수출',frequency:'T',unit:'억달러/조업일',category:'고빈도 경기',decimals:2,pending:'관세청 누계 수출액과 조업일수를 구간값으로 변환하는 수집 경로를 준비 중입니다.'},
...
function koTick(time){const p=dateParts(time);if(!p)return'';if(p.month===1&&p.day===1)return`${p.year}년`;if(p.day===1)return`${p.month}월`;return`${p.month}.${p.day}`;}
...
function fitMax(){const r=maxRange();if(r&&chart){chart.timeScale().setVisibleLogicalRange(r);placeBells();}}
function initialRange(){if(!rows.length)return null;const to=rows.length-1+rightGapBars();return{from:to-DEFAULT_VISIBLE_BARS,to};}
function pinLatestGap(){if(!chart||!rows.length)return;for(let i=0;i<2;i++){const r=chart.timeScale().getVisibleLogicalRange();if(!r)break;const width=r.to-r.from,to=rows.length-1+rightGapBars();changingRange=true;chart.timeScale().setVisibleLogicalRange({from:to-width,to});changingRange=false;}}
function showInitialRange(){const r=initialRange();if(r&&chart){chart.timeScale().setVisibleLogicalRange(r);pinLatestGap();}}
function clampRange(r){if(!r||!chart||!rows.length||changingRange)return;const last=rows.length-1,maxTo=last+rightGapBars(),w=r.to-r.from;let from=r.from,to=r.to;if(to>maxTo||(to>=last&&to<maxTo)){to=maxTo;from=to-w;}if(Math.abs(from-r.from)>.01||Math.abs(to-r.to)>.01){changingRange=true;chart.timeScale().setVisibleLogicalRange({from,to});changingRange=false;}placeBells();}
function wheel(e){if(!chart||!rows.length)return;e.preventDefault();const r=chart.timeScale().getVisibleLogicalRange()||initialRange(),last=rows.length-1,maxTo=last+rightGapBars(),w=Math.max(MIN_VISIBLE_BARS,r.to-r.from),nextW=Math.max(MIN_VISIBLE_BARS,w*(e.deltaY>0?1.16:.86)),anchor=r.to>=last?maxTo:Math.min(r.to,maxTo);changingRange=true;chart.timeScale().setVisibleLogicalRange({from:anchor-nextW,to:anchor});changingRange=false;placeBells();}
...
function syncAlertLines(){if(!meta||!raw)return;alertsFor(meta.code).forEach(t=>addLine(Number(t.target_value),t,false));placeBells();}
