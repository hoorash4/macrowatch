(() => {
  ...
  function render(container, rows, selectedYears) {
    ...
    const updateVisibleScale = () => {
      ...
      yLabels.forEach((label) => { const value = Number(label.dataset.koreaForeignFlowYMultiple) * current.tickStep; label.textContent = chartUtils.formatChartNumber(value, { showPlus: true }); });
    };
