(() => {
  ...
  function renderExtremeNewsSignals(rows) {
    ...
    document.querySelectorAll('#news-extreme-signals [data-extreme-signal-status]').forEach((element) => {
      element.textContent = '(월요일 자정 초기화)';
    });
  }
  ...
  function renderSentimentSegment(percent, colorClass, showLabel) {
    if (!percent) return '';
    const label = showLabel && percent >= 16
      ? `<span class="news-sentiment-segment-value text-[9px] font-bold text-white/90">${Math.round(percent)}%</span>`
      : '';
  ...
  function renderHorizontalSentimentSegment(percent, colorClass, showLabel) {
    if (!percent) return '';
    const label = showLabel && percent >= 12
      ? `<span class="sentiment-segment-label text-[10px] font-bold">${Math.round(percent)}%</span>`
      : '';
  ...
  function renderHorizontalSentimentBar(item, positive, negative, directionalCount, view, title) {
    ...
    const date = view.showDates
      ? `<span class="news-sentiment-row-date w-10 shrink-0 text-right text-xs font-semibold text-slate-600">${formatNewsDate(item.article_date)}</span>`
      : '';
    return (
      `<div class="news-sentiment-row group flex w-full items-center gap-3"${title ? ` title="${title}"` : ''}>` +
      `${date}` +
      `<div class="news-sentiment-horizontal-bar flex h-12 min-w-0 flex-1 overflow-hidden rounded-lg bg-slate-200/80 ring-1 ring-inset ring-slate-300 shadow-sm">${bar}</div>` +
      `</div>`
    );
  }
  ...
  function renderVerticalSentimentBar(item, positive, negative, directionalCount, view, title) {
    const bar = directionalCount
      ...
      : '<span class="m-auto text-[9px] font-semibold text-slate-500">—</span>';
    const date = view.showDates
      ? `<span class="whitespace-nowrap text-[10px] text-slate-600">${formatNewsDate(item.article_date)}</span>`
      : '';
    return (
      `<div class="group flex ${view.barWidthClass} flex-none flex-col items-center gap-2"${title ? ` title="${title}"` : ''}>` +
      `<div class="flex h-44 w-full flex-col overflow-hidden rounded-lg bg-slate-200/80 ring-1 ring-inset ring-slate-300 shadow-sm">${bar}</div>` +
      `${date}` +
      `</div>`
    );
  }
  ...
  async function loadNewsSentimentDashboard() {
    ...
    try {
      const { data, error } = await supabaseClient
        .from('news_daily_article_sentiment')
  ...
  function renderMarketStressDashboard(rows, weeklyRows = []) {
    ...
    const sp500Domain = hasSp500
      ? window.MacroWatchAnalysisChart.axisDomain(sp500Values, { minimumSpan: 1 })
      : { min: 0, max: 1 };
    ...
    const sp500Y = (value) => Number.isFinite(value)
      ? padding.top + ((height - padding.top - padding.bottom) * (sp500AxisMaximum - value)) / sp500AxisRange
      : NaN;
    ...
    const lines = monotoneStyledSegments(
      data,
      (_, index) => x(index),
      (row) => y(Number(row.stress_index)),
      (previous, row) => ({
        stroke: '#b7791f',
        width: lineWidths.primary,
        dash: row.is_provisional || previous.is_provisional ? '5 5' : ''
      }),
    );

    const dots = data.map((row, index) => {
      const provisional = Boolean(row.is_provisional);
      const detail = `${row.month}\nUS-MSI: ${formatChartNumber(row.stress_index, { maximumFractionDigits: 1 })}${provisional ? ' (잠정치)' : ' (확정치)'}`;
    ...
    const grid = scoreDomain.ticks.map((score) => (
      `<line data-chart-grid x1="${padding.left}" x2="${width - padding.right}" y1="${y(score)}" y2="${y(score)}" stroke="#dbe3ed" stroke-dasharray="3 4"/>` +
      `<text data-chart-left-axis x="${padding.left - 9}" y="${y(score) + 3}" text-anchor="end" fill="#64748b" font-size="10">${window.MacroWatchAnalysisChart.formatAxisNumber(score)}</text>`
    )).join('');

    chart.innerHTML = (
      `<div class="rounded-xl border border-slate-200 bg-slate-50 p-3">` +
      `<svg class="w-full" style="height:${height}px" viewBox="0 0 ${width} ${height}" role="img" aria-label="미국 시장 스트레스 지수와 S&P 500 월말 종가 추이">` +
      `<line x1="${padding.left}" x2="${padding.left}" y1="${padding.top}" y2="${height - padding.bottom}" stroke="#94a3b8"/>` +
      `<line x1="${width - padding.right}" x2="${width - padding.right}" y1="${padding.top}" y2="${height - padding.bottom}" stroke="#94a3b8"/>` +
      `${grid}${sp500Axis}${lines}${sp500Lines}${dots}${sp500Dots}${labels}` +
      `</svg>` +
      `</div>` +
      `${correlation == null ? '' : `<p class="mt-2 text-right text-[11px] text-slate-500">US-MSI·S&P 500 동일 월 상관계수: r = ${formatChartNumber(correlation)}</p>`}`
    );
  ...
  function renderMarketStressAndTensionChart(weeklyRows) {
    ...
    const width = window.MacroWatchAnalysisChart.historyWidth(weekly, 'week', usStressRangeYears);
    const height = CREDIT_STRESS_CHART_HEIGHT;
    const padding = chartPadding(US_MSI_PROFILE.axisMode);
    ...
    const tensionDomain = window.MacroWatchAnalysisChart.axisDomain(values, { minimumSpan: 1 });
    const { min: lower, max: upper } = tensionDomain;
    ...
    const { min: sp500Lower, max: sp500Upper } = sp500Domain;
    const sp500Y = (value) => Number.isFinite(value)
      ? padding.top + ((height - padding.top - padding.bottom) * (sp500Upper - value)) / Math.max(1, sp500Upper - sp500Lower)
      : NaN;
    ...
    const grid = [...tensionDomain.ticks].reverse().map((value) => {
      const py = y(value);
    ...
    const weeklyPaths = monotoneStyledSegments(
      weekly,
      (row) => x(row.week),
      (row) => y(Number(row.tension_index)),
      (previous, row) => {
    ...
    window.MacroWatchAnalysisChart.scrollableSvg(chart.querySelector('svg'), width, 920, {
      ...
      axes: [
        {
          points: weekly.map((row) => ({ x: x(row.week), value: toCreditStressNumber(row.tension_index) })),
          y,
          selector: 'path[stroke="#00838c"],path[stroke="#d97706"]'
        },
        {
          points: weekly.map((row) => ({ x: x(row.week), value: toCreditStressNumber(row.sp500_friday_close) })),
          y: sp500Y,
          side: 'right',
          selector: 'path[stroke="#6b7280"]'
        },
      ],
    });
    ...
    window.MacroWatchAnalysisChart.attachChartCursor({
      ...
      valueText: (row) => formatChartNumber(row.tension_index),
      dateText: (row) => {
        const [, month, day] = String(row.week).split('-').map(Number);
        return `${month}월 ${Math.ceil(day / 7)}주`;
      },
      eventName: 'macrowatch:market-stress-hover',
      source: 'stress',
      eventDetail: (row) => ({ week: row.week }),
      rowFromDetail: (detail) => weekly.find((row) => row.week === detail.week),
    });
  }

  function renderWeeklyMomentumChart({
    chartId,
    rows,
    valueKey,
    source,
    ...
    const width = chartId === 'credit-stress-momentum-chart'
      ? window.MacroWatchAnalysisChart.historyWidth(rows, 'month', usStressRangeYears)
      : 920;
    ...
    const grid = momentumDomain.ticks.map((value) => (
      `<line data-chart-grid x1="${padding.left}" x2="${width - padding.right}" y1="${y(value)}" y2="${y(value)}"${value === 0 ? ' class="analysis-chart-zero-line"' : ' stroke="#dbe3ed" stroke-dasharray="3 4"'}/>` +
      `<text data-chart-left-axis x="${padding.left - 8}" y="${y(value) + 3}" text-anchor="end" fill="#64748b" font-size="10">${formatAxisValue(value)}</text>`
    )).join('');

    const lines = showChanges
      ? `<path d="${monotoneSeriesPath(data, (row) => x(row.month), (row) => y(row.value))}" fill="none" stroke="${lineColor}" stroke-width="${lineWidths.comparison}" stroke-linecap="round"/>`
      : '';
    ...
    const secondaryAverageLines = secondaryAverageColor
      ? `<path d="${monotoneSeriesPath(data, (row) => x(row.month), (row) => y(row.secondaryAverage))}" fill="none" stroke="${secondaryAverageColor}" stroke-width="${lineWidths.auxiliary}" stroke-opacity="0.48" stroke-linecap="round"/>`
      : '';

    const yearGuides = data
      .filter((row, index) => index > 0 && String(row.month).slice(0, 4) !== String(data[index - 1].month).slice(0, 4))
      .map((row) => `<line x1="${x(row.month)}" x2="${x(row.month)}" y1="${padding.top}" y2="${height - padding.bottom}" stroke="#d4dde8" stroke-dasharray="3 4"/>`)
      .join('');
    ...
    window.MacroWatchAnalysisChart.attachChartCursor({
      ...
      eventName: 'macrowatch:market-stress-hover',
      source,
      eventDetail: (row) => ({ week: row.month }),
      rowFromDetail: (detail) => data.find((row) => row.month === detail.week),
    });
  }
  ...
  async function loadMarketTension(monthlyRows = []) {
    ...
    const [weeklyResponse, credit, risk, sp500] = await Promise.all([
      supabaseClient.from('us_market_tension_weekly')
        .select('week,tension_index,is_provisional')
        .order('week', { ascending: false })
        .limit(STRESS_HISTORY_QUERY_LIMIT),
      loadEconomicSeries('NFCI_CREDIT'),
      loadEconomicSeries('NFCI_RISK'),
      loadEconomicSeries('SP500_WEEKLY_CLOSE'),
    ]);
    if (weeklyResponse.error) return;

    const selectedWeeklyRows = (weeklyResponse.data || []).map((row) => ({
      ...row,
  ...
  async function loadMarketStressDashboard() {
    ...
    try {
      const [{ data, error }, sp500] = await Promise.all([
        supabaseClient.from('us_market_stress_index_monthly')
      ...
      const selectedRows = (data || []).map((row) => ({
        ...row,
        sp500_month_end_close: sp500.get(String(row.month))
      }));
  ...
  function renderEmStressDashboard(rows) {
    ...
    const width = window.MacroWatchAnalysisChart.historyWidth(weekly, 'week', emStressRangeYears);
    const height = CREDIT_STRESS_CHART_HEIGHT;
    const padding = chartPadding(EM_MSI_PROFILE.axisMode);
    const dates = weekly.map((row) => new Date(row.week).getTime());
    const start = Math.min(...dates);
    const end = Math.max(...dates);
    const x = (value) => padding.left + ((new Date(value).getTime() - start) / Math.max(1, end - start)) * (width - padding.left - padding.right);
    const values = weekly.map((row) => Number(row.stress_index));
    const stressDomain = window.MacroWatchAnalysisChart.axisDomain(values, { minimumSpan: 1 });
    const { min: lower, max: upper } = stressDomain;
    ...
    const eemDomain = hasEem
      ? window.MacroWatchAnalysisChart.axisDomain(eemValues, { minimumSpan: 1 })
      : { min: 0, max: 1 };
    const eemLower = eemDomain.min;
    const eemUpper = eemDomain.max;
    ...
    const grid = [...stressDomain.ticks].reverse().map((value) => (
      `<line data-chart-grid x1="${padding.left}" x2="${width - padding.right}" y1="${y(value)}" y2="${y(value)}" stroke="#dbe3ed" stroke-dasharray="3 4"/>` +
      `<text data-chart-left-axis x="${padding.left - 9}" y="${y(value) + 3}" text-anchor="end" fill="#64748b" font-size="10">${window.MacroWatchAnalysisChart.formatAxisNumber(value)}</text>`
    )).join('');

    const eem = hasEem
      ? `<path d="${monotoneSeriesPath(weekly, (row) => x(row.week), (row) => eemY(Number(row.eem_weekly_close)))}" fill="none" stroke="#6b7280" stroke-width="${lineWidths.comparison}" stroke-linecap="round"/>`
      : '';
    const eemLabels = hasEem
      ? window.MacroWatchAnalysisChart
    ...
    window.MacroWatchAnalysisChart.scrollableSvg(chart.querySelector('svg'), width, 920, {
      ...
      axes: [
        {
          points: weekly.map((row) => ({ x: x(row.week), value: toCreditStressNumber(row.stress_index) })),
          y,
          selector: 'path[stroke="#00838c"],path[stroke="#d97706"]'
        },
        {
          points: weekly.map((row) => ({ x: x(row.week), value: toCreditStressNumber(row.eem_weekly_close) })),
          y: eemY,
          side: 'right',
          selector: 'path[stroke="#6b7280"]'
        },
    ...
    window.MacroWatchAnalysisChart.attachChartCursor({
      ...
      top: padding.top,
      bottom: padding.bottom,
      valueText: (row) => formatChartNumber(row.stress_index),
      dateText: (row) => {
        const [, month, day] = String(row.week).split('-').map(Number);
        return `${month}월 ${Math.ceil(day / 7)}주`;
      },
  ...
  async function loadEmStressDashboard() {
    ...
    try {
      const [{ data, error }, eem] = await Promise.all([
        supabaseClient.from('em_market_stress_weekly')
          .select('week,stress_index,is_provisional')
          .order('week', { ascending: false })
          .limit(STRESS_HISTORY_QUERY_LIMIT),
        loadEconomicSeries('EEM_WEEKLY_CLOSE'),
      ]);
      if (error) throw error;
      renderEmStressDashboard((data || []).map((row) => ({
        ...row,
        eem_weekly_close: eem.get(String(row.week))
      })));
    } catch (_) {
  ...
  function renderKoreaStressChart(rows, weeklyKospiRows = []) {
    ...
    const width = window.MacroWatchAnalysisChart.historyWidth(data, 'month', koreaStressRangeYears);
    const height = CREDIT_STRESS_CHART_HEIGHT;
    const padding = chartPadding(KOREA_MSI_PROFILE.axisMode);
    const dates = [
      ...data.map((row) => new Date(row.month).getTime()),
      ...weeklyKospi.map((row) => new Date(row.week).getTime())
    ];
    const start = Math.min(...dates);
    const end = Math.max(...dates);
    ...
    const stressDomain = window.MacroWatchAnalysisChart.axisDomain(leftValues, { minimumSpan: 1 });
    const { min: lower, max: upper } = stressDomain;
    const y = (value) => padding.top + (height - padding.top - padding.bottom) * (upper - value) / Math.max(1, upper - lower);
    const kospiValues = weeklyKospi.map((row) => Number(row.kospi_close)).filter(Number.isFinite);
    const hasKospi = kospiValues.length > 1;

    const kospiDomain = hasKospi
      ? window.MacroWatchAnalysisChart.axisDomain(kospiValues, { minimumSpan: 1 })
      : { min: 0, max: 1 };
    const kospiLower = kospiDomain.min;
    const kospiUpper = kospiDomain.max;
    ...
    const kospi = hasKospi
      ? `<path d="${monotoneSeriesPath(weeklyKospi, (row) => x(row.week), (row) => kospiY(Number(row.kospi_close)))}" fill="none" stroke="#6b7280" stroke-width="${lineWidths.comparison}" stroke-linecap="round"/>`
      : '';

    const kospiLabels = hasKospi
      ? window.MacroWatchAnalysisChart
    ...
    window.MacroWatchAnalysisChart.scrollableSvg(chart.querySelector('svg'), width, 920, {
      ...
      axes: [
        {
          points: data.map((row) => ({ x: x(row.month), value: toCreditStressNumber(row.stress_index) })),
          y,
          selector: 'path[stroke="#00838c"],path[stroke="#d97706"]'
        },
        {
          points: weeklyKospi.map((row) => ({ x: x(row.week), value: toCreditStressNumber(row.kospi_close) })),
          y: kospiY,
          side: 'right',
          selector: 'path[stroke="#6b7280"]'
        },
    ...
    window.MacroWatchAnalysisChart.attachChartCursor({
      ...
      bottom: padding.bottom,
      valueText: (row) => formatChartNumber(row.stress_index),
      dateText: (row) => `${Number(String(row.month).slice(5, 7))}월`,
      eventName: 'macrowatch:korea-stress-hover',
      source: 'korea-main',
      eventDetail: (row) => ({ month: row.month }),
    ...
    const fsiHeight = chartLayout.auxiliaryHeight;
    const fsiPadding = chartPadding('dual');
    const fsiValues = fsiRows.map((row) => Number(row.bok_fsi));
    const fsiDomain = window.MacroWatchAnalysisChart.axisDomain(fsiValues, { minimumSpan: 1, targetIntervals: 3 });
    const { min: fsiLower, max: fsiUpper } = fsiDomain;
    const fsiY = (value) => fsiPadding.top + (fsiHeight - fsiPadding.top - fsiPadding.bottom) * (fsiUpper - value) / Math.max(1, fsiUpper - fsiLower);

    const fsiGrid = [...fsiDomain.ticks].reverse().map((chartValue) => (
      `<line data-chart-grid x1="${fsiPadding.left}" x2="${width - fsiPadding.right}" y1="${fsiY(chartValue)}" y2="${fsiY(chartValue)}" stroke="#e6e1f2" stroke-dasharray="3 4"/>` +
      `<text data-chart-left-axis x="${fsiPadding.left - 9}" y="${fsiY(chartValue) + 3}" text-anchor="end" fill="#7c6b9d" font-size="10">${window.MacroWatchAnalysisChart.formatAxisNumber(chartValue)}</text>`
    )).join('');
    ...
    window.MacroWatchAnalysisChart.scrollableSvg(fsiChart.querySelector('svg'), width, 920, {
      left: fsiPadding.left,
      right: fsiPadding.right,
      axisMode: fsiPadding.axisMode,
      xAxisMode: 'none',
      showScrollbar: false,
      top: fsiPadding.top,
      bottom: fsiHeight - fsiPadding.bottom,
      axes: [
        {
          points: fsiRows.map((row) => ({ x: x(row.month), value: toCreditStressNumber(row.bok_fsi) })),
          y: fsiY,
          selector: 'path[stroke="#6d4b91"]'
        },
      ]
    });
    ...
    window.MacroWatchAnalysisChart.attachChartCursor({
      ...
      height: fsiHeight,
      top: fsiPadding.top,
      bottom: fsiPadding.bottom,
      valueText: null,
      dateText: null,
      eventName: 'macrowatch:korea-stress-hover',
      source: 'korea-fsi',
      eventDetail: (row) => ({ month: row.month }),
      rowFromDetail: (detail) => fsiRows.find((row) => row.month === detail.month),
    });
  }
    ...
    try {
      const [monthlyResponse, bokFsi, kospiWeekly] = await Promise.all([
        supabaseClient.from('korea_market_stress_monthly')
          ...
          .order('month', { ascending: false })
          .limit(STRESS_HISTORY_QUERY_LIMIT),
        loadEconomicSeries('BOK_FSI'),
        loadEconomicSeries('KOSPI_WEEKLY_CLOSE'),
      ]);
      if (monthlyResponse.error) throw monthlyResponse.error;
      let displayRows = (monthlyResponse.data || []).map((row) => ({
        ...row,
        bok_fsi: bokFsi.get(String(row.month))
      }));
      if (!displayRows.some((row) => Number.isFinite(Number(row.bok_fsi)))) {
        try {
          ...
          displayRows = displayRows.map((row) => ({
            ...row,
            bok_fsi: officialFsi[String(row.month).slice(0, 10)] ?? null
          }));
        } catch (_) {
  ...
  function sectorReturn(value) {
    ...
    return `${formatChartNumber(number, { maximumFractionDigits: digits, showPlus: true })}%`;
  }
  ...
  function initializeSectorHoldingInteractions() {
    ...
    dashboard.addEventListener('keydown', (event) => {
      if (event.key === 'Escape') {
        closeSectorHoldings(event.target.closest('.sector-flow-sector.is-open'));
      }
    });
  }
  ...
  function renderSectorFlow(rows) {
    ...
    cards.forEach((card) => {
      ...
      const week = weeks[weeks.length - 1 + offset];
      const isLatestWeek = offset === 0;
      ...
      if (columns) {
        columns.innerHTML = isLatestWeek
          ? isMobile
  ...
  window.MacroWatchChartUtils = Object.freeze({
    aggregateWeeklyDecisiveNews,
    calculateCorrelation,
    formatNewsDate
  });

  bindStressRangeControls(
    '[data-us-stress-ranges]',
    'data-us-stress-range',
    () => usStressRangeYears,
    (value) => { usStressRangeYears = value; },
    loadMarketStressDashboard
  );

  bindStressRangeControls(
    '[data-korea-stress-ranges]',
    'data-korea-stress-range',
    () => koreaStressRangeYears,
    (value) => { koreaStressRangeYears = value; },
    loadKoreaStressDashboard
  );

  bindStressRangeControls(
    '[data-em-stress-ranges]',
    'data-em-stress-range',
    () => emStressRangeYears,
    (value) => { emStressRangeYears = value; },
