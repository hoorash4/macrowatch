(() => {
  ...
  const invoke = (name, action, payload = {}) => {
    ...
    return functions.invoke(name, { action, ...payload }, { errorMessage: status => `${name} 요청에 실패했습니다. (${status})` });
  };
  ...
  function setKakaoStatus(connected, message) {
    ...
    badge.className = connected ? 'shrink-0 rounded-full border border-emerald-700/50 bg-emerald-950/60 px-2.5 py-1 text-[11px] font-semibold text-emerald-400' : 'shrink-0 rounded-full border border-slate-700 bg-slate-800 px-2.5 py-1 text-[11px] font-semibold text-slate-400';
  ...
  async function loadKakaoStatus() {
    ...
    try { const data = await invoke('kakao-auth', 'status'); setKakaoStatus(Boolean(data?.connected)); }
  ...
  function setEmailStatus(address, active, message) {
    ...
    badge.className = active ? 'shrink-0 rounded-full border border-emerald-700/50 bg-emerald-950/60 px-2.5 py-1 text-[11px] font-semibold text-emerald-400' : 'shrink-0 rounded-full border border-slate-700 bg-slate-800 px-2.5 py-1 text-[11px] font-semibold text-slate-400';
    input.value = address || '';
    remove.classList.toggle('hidden', !active);
  }
  async function loadEmailStatus() {
    setEmailStatus('', false, '설정 상태 확인 중');
    try { const data = await invoke('notification-settings', 'status'); setEmailStatus(data?.address || '', Boolean(data?.is_active)); }
  ...
  async function loadProfileIdentity() {
    ...
    } catch { target.textContent = '아이디를 확인하지 못했습니다.'; }
  }
  ...
  function bind() {
    ...
    if (!client || !modal || modal.dataset.accountControlsBound === 'true') return;
    modal.dataset.accountControlsBound = 'true';
    const close = () => { modal.classList.add('hidden'); document.getElementById('profile-button')?.focus(); };
    ...
    document.getElementById('kakao-unlink-button')?.addEventListener('click', async event => {
      ...
      try { await invoke('kakao-auth', 'unlink'); await loadKakaoStatus(); } catch (error) { window.alert(error.message || '카카오 연결을 해제하지 못했습니다.'); } finally { event.currentTarget.disabled = false; }
    });
    document.getElementById('email-alert-save-button')?.addEventListener('click', async event => {
      event.currentTarget.disabled = true;
      try { await invoke('notification-settings', 'save', { address: document.getElementById('email-alert-address')?.value.trim() || '' }); await loadEmailStatus(); } catch (error) { window.alert(error.message || '이메일 알림을 저장하지 못했습니다.'); } finally { event.currentTarget.disabled = false; }
    });
    document.getElementById('email-alert-remove-button')?.addEventListener('click', async event => {
      ...
      event.currentTarget.disabled = true;
      try { await invoke('notification-settings', 'remove'); await loadEmailStatus(); } catch (error) { window.alert(error.message || '이메일 알림을 해제하지 못했습니다.'); } finally { event.currentTarget.disabled = false; }
    });
    document.getElementById('password-change-form')?.addEventListener('submit', async event => {
      ...
      event.currentTarget.reset(); window.alert('비밀번호를 변경했습니다.');
    });
    ...
    document.getElementById('account-delete-confirm')?.addEventListener('click', async event => {
      const button = event.currentTarget; button.disabled = true; button.textContent = '처리 중';
      try { await invoke('kakao-auth', 'delete_account'); await client.auth.signOut({ scope: 'local' }); window.location.replace('./'); }
      catch (error) { window.alert(error.message || '회원 탈퇴를 처리하지 못했습니다.'); button.disabled = false; button.textContent = '탈퇴하기'; }
    });
    ...
    document.getElementById('service-preparing-modal')?.addEventListener('click', event => { if (event.target === event.currentTarget) event.currentTarget.classList.add('hidden'); });
    updateAdminLink().catch(error => console.warn('admin access check failed', error));
  }
  window.MacroWatchAccountControls = Object.freeze({ bind, updateAdminLink });
