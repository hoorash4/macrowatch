"""Public Equifax small-business delinquency/default source.

MacroWatch stores only two series from Equifax:
- US_SBDI_31_180: 31-90 Days Past Due + 91-180 Days Past Due
- US_SBDFI: published Small Business Default Index level

The split delinquency buckets are parsing inputs only and are never stored as chart series.
"""
...
def extract_split_levels(text: str) -> tuple[float, float, float] | None:
...
def _fetch_report(report_year: int, report_month: int) -> tuple[float, float, float, str] | None:
    for url in _report_urls(report_year, report_month):
        try:
            ...
            levels = extract_split_levels(text)
...
def fetch_equifax_month(observed: date) -> dict[str, dict | None]:
    """Fetch one observation month and emit only unified delinquency + default rows."""
...
def fetch_equifax_rows(start: date, end: date) -> dict[str, list[dict]]:
    ...
    while current >= first:
        rows = fetch_equifax_month(current)
