...
always the sum of the two non-overlapping published SBDI buckets.
"""
...
def _fetch_report_text(report_year: int, report_month: int) -> tuple[tuple[float, float, float], str, str] | None:
    """Return exact current levels, normalized report text and the official URL."""
...
def _directional_delta(body: str, period: str) -> float | None:
    """Extract a signed percentage-point change for M/M or Y/Y from one metric block."""
    p = re.escape(period)
    patterns = (
    ...
    # Narrative legacy format: "15 bps above/below ... year ago/last month".
    if period.upper() == "Y/Y":
        narrative = re.search(
            r"([0-9]+(?:\.[0-9]+)?)\s*(bps?|bp|pp|percentage\s+points?)\s*"
            r"(above|higher\s+than|up\s+from|below|lower\s+than|down\s+from)[^.]{0,90}?"
            r"(?:year[- ]ago|a\s+year\s+ago|last\s+year|year[- ]over[- ]year)",
            body, re.I,
        )
    else:
        narrative = re.search(
            r"([0-9]+(?:\.[0-9]+)?)\s*(bps?|bp|pp|percentage\s+points?)\s*"
            r"(above|higher\s+than|up\s+from|below|lower\s+than|down\s+from)[^.]{0,90}?"
            r"(?:last\s+month|previous\s+month|month[- ]ago|month[- ]over[- ]month)",
            body, re.I,
        )
    if narrative:
...
def _change_map(clean: str, period: str) -> dict[str, float]:
    """Extract signed deltas for the three published metrics without crossing metric blocks."""
    ...
    out: dict[str, float] = {}
    for idx, (start, end, token) in enumerate(starts):
...
def _derive_from_report(target: date, *, lag_months: int, period: str) -> dict[str, dict | None]:
    """Derive target from a later observation's published change and exact current level."""
...
def fetch_paynet_derived_month(observed: date) -> dict[str, dict | None]:
    """Fill a direct-source gap from official Equifax changes, preferring YoY then MoM."""
